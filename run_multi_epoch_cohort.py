...
def _execute_epoch(
    ...
    return {
        ...
        "epoch_wall_clock_start_ns": wall_clock_start_ns,
        "epoch_wall_clock_end_ns": wall_clock_end_ns,
        "native_wall_us": result.wall_us,
        "native_cpu_us": result.native_cpu_us,
        "declared_input_bytes": result.declared_input_bytes,
        "declared_output_bytes": result.declared_output_bytes,
        "declared_total_bytes": result.declared_input_bytes
        + result.declared_output_bytes,
        "socket_send_calls": result.socket_send_calls,
        "socket_receive_calls": result.socket_receive_calls,
...
def run_dynamic_trace(
    ...
    for index, epoch in enumerate(epochs):
        ...
        checks = {
            ...
            "one_socket_round_trip": epoch["socket_send_calls"] == 1
            and epoch["socket_receive_calls"] == 1,
            "two_model_calls": epoch["model_calls"] == 2,
            "minimal_abi": epoch["declared_input_bytes"] == 260
            and epoch["declared_output_bytes"] == 368,
            "native_cpu_recorded": epoch["native_cpu_us"] > 0,
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        "gate": "Attempt 74 minimal ABI multi-epoch residency",
        "schema_version": 1,
        "pass": False,
        "support_boundary": {
    ...
    try:
        ...
        if worker.resident_config.max_steps != 2:
            raise RuntimeError("Attempt 74 requires a two-step resident epoch")
        if worker.resident_config.logical_capacity != 8:
            raise RuntimeError("Attempt 74 requires logical capacity eight")
        ...
        result["warmup"] = {
            ...
            "native_cpu_us": worker.warmup_output.native_cpu_us,
            "declared_input_bytes": worker.warmup_output.declared_input_bytes,
            "declared_output_bytes": worker.warmup_output.declared_output_bytes,
        }
        ...
        result["checks"] = {
            ...
            "warmup_one_complete_step": result["warmup"]["status"] == 0
            and result["warmup"]["model_calls"] == 1
            and result["warmup"]["feed_calls"] == 1
            and result["warmup"]["fetch_calls"] == 1
            and result["warmup"]["wall_us"] > 0,
            "warmup_minimal_abi": result["warmup"]["declared_input_bytes"]
            == 260
            and result["warmup"]["declared_output_bytes"] == 368
            and result["warmup"]["native_cpu_us"] > 0,

# G4a Attempt 53f: HF-Scaling Complete-Decoder Eager Control

Date frozen: 2026-07-23

## Claim

This controlled eager experiment tests whether the step-4 logits drift comes
from using FP32 division instead of the exact Qwen2 eager scaling expression.
...
- Native `torch.matmul` remains as in Attempt 53e.
- The only new change is replacing `raw.float() / sqrt(128) -> BF16` with
  Qwen2's `raw * 128**-0.5` BF16 expression before the barrier.

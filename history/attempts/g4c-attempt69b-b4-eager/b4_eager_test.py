...
def case_specs(token_ids: np.ndarray) -> list[dict]:
    return [
        {
            "name": "all-active-heterogeneous",
            "token": token_ids.copy().reshape(BATCH_SIZE, 1),
            "position": np.asarray([0, 1, 2, 3], dtype=np.int64),
            "sequence_length": np.asarray([[1], [2], [3], [4]], dtype=np.int32),
            "active_mask": np.asarray([1, 1, 1, 1], dtype=np.int32),
        },
        {
            "name": "two-active-two-empty",
            "token": np.asarray(
                [[token_ids[0]], [0], [token_ids[2]], [0]], dtype=np.int64
            ),
            "position": np.asarray([1, 0, 2, 0], dtype=np.int64),
            "sequence_length": np.asarray([[2], [0], [3], [0]], dtype=np.int32),
            "active_mask": np.asarray([1, 0, 1, 0], dtype=np.int32),
        },
        {
            "name": "finished-active-empty-active",
            "token": np.asarray(
                [[token_ids[3]], [token_ids[1]], [0], [token_ids[2]]],
                dtype=np.int64,
            ),
            "position": np.asarray([3, 1, 0, 2], dtype=np.int64),
            "sequence_length": np.asarray([[4], [2], [0], [3]], dtype=np.int32),
            "active_mask": np.asarray([0, 1, 0, 1], dtype=np.int32),
        },
    ]
...
def main() -> None:
    ...
    reference_path = args.output_dir / "attempt69b-b4-eager-reference.npz"
    np.savez(reference_path, **archive)
    result = {
        "gate": "G4c Attempt 69b B=4 batched eager versus independent B=1 oracle",
        ...
        "claim_boundary": (
            "This closes only B=4 eager batching semantics. AIR, native GE, "
            "Device UDF, performance, recovery, and vLLM integration "
            "remain open."
        ),
    }
    result_path = args.output_dir / "attempt69b-b4-eager-result.json"
    result_path.write_text(
        json.dumps(result, indent=2, ensure_ascii=True) + "\n", encoding="utf-8"
    )
    print("G4C_ATTEMPT69B " + json.dumps(result, ensure_ascii=True), flush=True)
    if not result["pass"]:
        raise SystemExit(1)

...
raw=${root}/raw-attempt46
cache_dir=${root}/cache-attempt46
...
protocol=${root}/attempt46-protocol.md

expected_package_sha=47be1f718dffd57fd8f0328032e7165d860e6f51d20e955ba1f13503cea3762c
...
expected_protocol_sha=cf9f217b11de332efb5faab721cdf20d75bb0c18c83506ce801771679049b79d

if [[ -e "${raw}" || -e "${cache_dir}" ]]; then
  printf 'G2G_ATTEMPT46_REFUSE_OVERWRITE raw=%s cache=%s\n' \
    "${raw}" "${cache_dir}"
  exit 97
fi
...
sha256sum "${package}" "${installed_source}" "${air}" "${host_source}" \
  ...
  "${root}/run_g2g_attempt46.sh" >"${raw}/artifact-integrity.log"
...
if [[ $(wc -l <"${raw}/exactqk-launch-metadata.txt") -eq 1 ]] &&
   grep -Fq 'te_exactqk' "${raw}/exactqk-launch-metadata.txt" &&
  ...
  mode_status=0
  cp "${raw}/exactqk-launch-metadata.txt" "${raw}/exactqk-aic-mode0.txt"
else
  mode_status=1
fi
...
python "${comparator}" --native-dir "${native_output_dir}" \
  --eager-reference "${eager}" --output-npz "${raw}/attempt46-output.npz" \
  --output "${raw}/attempt46-result.json" >"${raw}/compare.stdout.log" 2>&1
compare_status=$?
...
sha256sum "${air}" "${raw}/attempt46-output.npz" \
  "${raw}/attempt46-result.json" >"${raw}/result-integrity.log"

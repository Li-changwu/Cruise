...
def main() -> None:
    ...
    active_data = [item for item in nodes if item[1] == "Data" and re.search(r"arg8_", item[0])]
    active_consumers = []
    if len(active_data) == 1:
        active_name = active_data[0][0]
        for name, op, block in nodes:
            inputs = re.findall(r'^\s+input: "([^"]*)"', block, re.MULTILINE)
            if any(value.split(":", 1)[0] == active_name for value in inputs):
                active_consumers.append({"name": name, "op": op})
    result = {
        "valid": (
        ...
        "active_mask_direct_consumers": active_consumers,

...
int RunCase(const std::shared_ptr<ge::Session> &session,
  ...
  std::cout << "G4C_ATTEMPT67E_CASE case=" << case_index
            << " outputs=" << outputs.size() << std::endl;
  return 0;
}
}  // namespace
...
int main(int argc, char **argv) {
  ...
  ge::Graph graph("G4cAttempt67eB2Decoder");
  const auto load_status = graph.LoadFromFile(argv[1]);
  std::cout << "G4C_ATTEMPT67E_LOAD status=" << load_status
  ...
  session.reset();
  ge::GEFinalize();
  std::cout << "G4C_ATTEMPT67E_NATIVE_COMPLETE" << std::endl;
  return 0;
}

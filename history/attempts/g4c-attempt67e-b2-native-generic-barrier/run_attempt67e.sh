...
src=${root}/attempt67e-src
raw=${root}/raw-attempt67e-b2-native
build=${root}/build-attempt67e-b2-native
...
validation=${root}/validation-attempt67b-r2-b2

if [[ -e "${raw}" || -e "${build}" ]]; then exit 97; fi
if [[ ! -f "${air}" || ! -f "${reference}" ]] ||
   ! grep -q '"valid": true' "${validation}/abi.json" ||
   ! grep -q '"valid": true' "${validation}/graph-inspection.json"; then
  exit 96
fi
...
barrier_set_env=$(find "${root}/install-attempt67d-barrier" -type f -name set_env.bash | head -1)
...
sha256sum "${src}"/*.py "${src}"/*.cpp "${src}"/*.md "${src}"/*.sh \
  "${air}" "${reference}" "${validation}/abi.json" \
  "${validation}/graph-inspection.json" \
  "${root}/raw-attempt67d-barrier-build/result-integrity.log" \
  >"${raw}/input-integrity.log"

python "${src}/prepare_b2_inputs.py" --reference "${reference}" \
...
g++ -D_GLIBCXX_USE_CXX11_ABI=0 -O2 -std=c++11 -ftrapv \
  ...
  "${src}/native_b2_decoder_host.cpp" -Wl,--whole-archive \
  ...
  -o "${build}/native_b2_decoder_host" >"${raw}/compile.log" 2>&1
s=$?
printf 'compile\t%s\n' "${s}" >>"${status}"
[[ ${s} -eq 0 ]] || exit ${s}

timeout 3600 "${build}/native_b2_decoder_host" "${air}" "${inputs}" \
...
python "${src}/compare_b2_native.py" --native-dir "${outputs}" \
  --reference "${reference}" --output-npz "${raw}/attempt67e-native-output.npz" \
  --output "${raw}/attempt67e-result.json" >"${raw}/compare.stdout.log" 2>&1
compare_status=$?
...
if [[ ${exact_count} -eq 168 ]]; then exact_status=0; else exact_status=93; fi
...
if [[ ${barrier_count} -eq 84 ]]; then barrier_status=0; else barrier_status=92; fi
...
if [[ ${materialize_count} -eq 168 ]]; then materialize_status=0; else materialize_status=91; fi
...
python "${src}/extract_linear_launches.py" --log "${raw}/native.stdout.log" \
  --expected 591 --output "${raw}/linear-launches.json" \
...
sha256sum "${air}" "${raw}/attempt67e-native-output.npz" \
  "${raw}/attempt67e-result.json" "${raw}/linear-launches.json" \
  >"${raw}/result-integrity.log" 2>/dev/null || true
cat "${raw}/attempt67e-result.json" 2>/dev/null || true

# G4a Attempt 53g: No-Barrier Complete-Decoder Eager Control
...
from the per-layer eager implementation of `Bf16Barrier` changing NPU layout.
...
- Native `torch.matmul` and Qwen2 scaling remain as in Attempt 53f.
- The only new change is bypassing `g4a_barrier.materialize` after scaling.

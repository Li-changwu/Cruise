...
src=${root}/attempt66b-r4-src
raw=${root}/raw-attempt66b-r4
cache=${root}/cache-attempt66b-r4
build=${root}/build-attempt66b-r4
weights=${root}/external-weights-attempt66b-r4
...
python "${src}/compare_epochs.py" --host-dir "${host_outputs}" \
  ...
  --eager-reference "${reference}" --output "${raw}/attempt66b-r4-result.json" \
...
sha256sum "${raw}/attempt66b-r4-result.json" \
  >"${raw}/result-integrity.log" 2>/dev/null || true
cat "${raw}/attempt66b-r4-result.json" 2>/dev/null || \

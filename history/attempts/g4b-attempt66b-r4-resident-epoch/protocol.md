# G4b Attempt 66b-r4: B=1 device-resident real generation epoch
...
Attempt 66b-r1 stopped during Host runner compilation because `GetData()` was
cast from `const uint8_t*` with `static_cast`. Attempt 66b-r2 changed only that
conversion to `reinterpret_cast<const float*>`; its Host suite passed, while
the invoked GraphPp converted the BF16 cache input `arg13_1` to
`DT_UNDEFINED`. Attempt 66b-r3 changed the two GraphPp JSON descriptors from
`DT_BF16` to GE's printed spelling `DT_BFLOAT16`. That removed the dtype
failure and compiled the complete graph, but GraphPp converted FileConstant
weights into a 15,240,810,868-byte in-memory model. `GraphModel::Init` then
passed that model to `memcpy_s`, whose CANN build limits one copy to
`0x7fffffff` bytes.

Attempt 66b-r4 retains the r3 dtype fix and enables `ge.externalWeight=1` with
an experiment-isolated external-weight directory and model prefix. The sole
mechanism change is preserving or regenerating external weights so the
GraphPp metadata model stays below the single-copy limit. Graph, control,
input, tolerance and comparison semantics remain unchanged. The external
weight file list and total size are captured on every exit.

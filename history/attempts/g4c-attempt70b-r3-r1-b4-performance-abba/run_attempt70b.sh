...
src=${root}/attempt70b-r3-r1-src
evidence=${root}/evidence-attempt70b-r3-r1-b4-performance
scratch=/dev/shm/ascend-control-g4-20260726/attempt70b-r3-r1-b4-performance
...
prepared_runner=${G4_PREPARED_RUNNER_DIR:-/dev/shm/ascend-control-g4-20260726/attempt70b-r2-prepared-runner}
raw=${scratch}/raw
inputs=${raw}/inputs
...
# The formal retry requires a five-minute quiet window after the pinned baseline.
export STORAGE_GUARD_NPU_STABLE_SAMPLES=${G4_NPU_STABLE_SAMPLES:-60}
...
wait_npu_ready() {
  local label=$1
  storage_guard_wait_for_npu_idle 7 "${STORAGE_GUARD_NPU_WAIT_SECONDS}" "${root}" 24 \
...
run_step graph-lifetime-selftest 120s python3 "${src}/test_graph_lifetime.py"
lifetime_test_status=$?
[[ ${lifetime_test_status} -eq 0 ]] || exit ${lifetime_test_status}
...
run_step preserve-performance-rows 120s bash -c '
  set -euo pipefail
  evidence_perf=$1
  perf_outputs=$2
  mkdir -p "${evidence_perf}"
  cp "${perf_outputs}/h1/perf-block.tsv" "${evidence_perf}/h1.tsv"
  cp "${perf_outputs}/d1/perf-block.tsv" "${evidence_perf}/d1.tsv"
  cp "${perf_outputs}/d2/perf-block.tsv" "${evidence_perf}/d2.tsv"
  cp "${perf_outputs}/h2/perf-block.tsv" "${evidence_perf}/h2.tsv"
' _ "${evidence}/perf" "${perf_outputs}"
preserve_status=$?
[[ ${preserve_status} -eq 0 ]] || exit ${preserve_status}
...
run_step analyze-abba-performance 1800s python3 \
  ...
  --output "${evidence}/attempt70b-r3-r1-result.json"
analysis_status=$?
...
idle_status=$?
capture_after
sha256sum "${evidence}/attempt70b-r3-r1-result.json" "${status}" \
  "${evidence}/perf/h1.tsv" "${evidence}/perf/d1.tsv" \
  "${evidence}/perf/d2.tsv" "${evidence}/perf/h2.tsv" \
  >"${evidence}/result-integrity.log" 2>/dev/null || true
storage_guard_finalize
finalize_status=$?
[[ ${finalize_status} -eq 0 ]] || exit ${finalize_status}
finalized=1
cat "${evidence}/attempt70b-r3-r1-result.json" 2>/dev/null ||

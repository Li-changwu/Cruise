...
src=${root}/attempt69d-r1-src
impl_src=${root}/attempt69d-src
raw=${root}/raw-attempt69d-r1-b4-native
build=${root}/build-attempt69d-r1-b4-native
inputs=${raw}/native-inputs
outputs=${raw}/native-outputs
export_dir=${root}/export-attempt69c-r2-b4
air=${export_dir}/qwen_b4_decoder_step_attempt69c_r2.air
reference=${root}/raw-attempt69b-b4-eager/outputs/attempt69b-b4-eager-reference.npz
relocation_raw=${root}/raw-attempt69c-r2-b4-air-relocation
relocation=${relocation_raw}/attempt69c-r2-result.json
expected_air_sha=263b2acf291e13f6a84042ded53c8dccabb1fa847dcdcbbbe0ece418610ad1e3

if [[ -e "${raw}" || -e "${build}" ]]; then exit 97; fi
if [[ ! -f "${air}" || ! -f "${reference}" || ! -f "${relocation}" ]] ||
   ! grep -q '"pass": true' "${relocation}" ||
   ! awk 'NR > 1 && $2 != 0 {exit 1}' "${relocation_raw}/status.tsv"; then
  exit 96
fi
actual_air_sha=$(sha256sum "${air}" | awk '{print $1}')
...
[[ -n "${custom_set_env}" && -n "${barrier_set_env}" && -n "${materialize_set_env}" ]] || exit 93
...
sha256sum "${src}"/*.py "${src}"/*.md "${src}"/*.sh \
  "${impl_src}"/*.py "${impl_src}"/*.cpp "${impl_src}"/*.md \
  "${impl_src}"/*.sh "${air}" "${reference}" "${relocation}" \
  "${relocation_raw}/status.tsv" \
  "${root}/raw-attempt69a-b4-barrier-build/result-integrity.log" \
  >"${raw}/input-integrity.log"

python "${impl_src}/prepare_b4_inputs.py" --reference "${reference}" \
...
g++ -D_GLIBCXX_USE_CXX11_ABI=0 -O2 -std=c++11 -ftrapv \
  ...
  "${impl_src}/native_b4_decoder_host.cpp" -Wl,--whole-archive \
...
python "${impl_src}/compare_b4_native.py" --native-dir "${outputs}" \
  --reference "${reference}" --output-npz "${raw}/attempt69d-r1-native-output.npz" \
  --output "${raw}/attempt69d-r1-comparison.json" >"${raw}/compare.stdout.log" 2>&1
compare_status=$?
...
if [[ ${exact_count} -eq 112 ]]; then exact_status=0; else exact_status=92; fi
...
if [[ ${barrier_count} -eq 28 ]]; then barrier_status=0; else barrier_status=91; fi
...
if [[ ${materialize_count} -eq 56 ]]; then materialize_status=0; else materialize_status=90; fi
printf '%s\n' "${materialize_count}" >"${raw}/materialize-launch-count.txt"
printf 'materialize-launch\t%s\n' "${materialize_status}" >>"${status}"

python "${impl_src}/extract_linear_launches.py" --log "${raw}/native.stdout.log" \
...
python "${src}/accept_attempt69d_r1.py" \
  --comparison "${raw}/attempt69d-r1-comparison.json" \
  --relocation "${relocation}" --linear "${raw}/linear-launches.json" \
  --status "${status}" --air "${air}" --exact-count "${exact_count}" \
  --barrier-count "${barrier_count}" --materialize-count "${materialize_count}" \
  --output "${raw}/attempt69d-r1-acceptance.json" \
  >"${raw}/acceptance.stdout.log" 2>&1
acceptance_status=$?
printf 'acceptance\t%s\n' "${acceptance_status}" >>"${status}"
sha256sum "${air}" "${raw}/attempt69d-r1-native-output.npz" \
  "${raw}/attempt69d-r1-comparison.json" \
  "${raw}/attempt69d-r1-acceptance.json" "${raw}/linear-launches.json" \
  >"${raw}/result-integrity.log" 2>/dev/null || true
cat "${raw}/attempt69d-r1-acceptance.json" 2>/dev/null || true
...
if [[ ${acceptance_status} -ne 0 ]]; then exit ${acceptance_status}; fi
exit ${idle_status}

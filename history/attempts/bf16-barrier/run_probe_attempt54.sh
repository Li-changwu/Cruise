...
src=${g2g}/attempt54-probe-src
input=${g2e}/raw-attempt52c-native/native-outputs
export_dir=${g2g}/attempt54-export
raw=${g2g}/raw-attempt54-probe
cache=${g2g}/cache-attempt54-probe
...
s=$?; printf 'native\t%s\n' "$s" >>"${status}"; [[ $s -eq 0 ]] || exit $s
grep -i 'LaunchKernel: kernel info.*bf16_barrier' "${raw}/native.stdout.log" \

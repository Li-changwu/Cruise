...
def main() -> None:
    ...
    with np.load(args.reference) as reference:
        for case_index, case_name in enumerate(CASES):
            ...
            # Attempt 69b retained the historical `_b2_` archive key spelling;
            # its tensors and frozen reference are the accepted true B=4 run.
            ...
            flat_key = key.reshape(flat_shape)
            flat_value = value.reshape(flat_shape)
            flat_input_key = input_key.reshape(flat_shape)
            flat_input_value = input_value.reshape(flat_shape)
            inactive_cache_exact = []
            active_greedy_equal = []
            active_logits_finite = []
            for request in range(BATCH_SIZE):
                if active[request] == 0:
                    begin = request * BLOCKS_PER_REQUEST
                    end = begin + BLOCKS_PER_REQUEST
                    inactive_cache_exact.append(
                        bool(
                            np.array_equal(key[:, begin:end], input_key[:, begin:end])
                            and np.array_equal(
                                value[:, begin:end], input_value[:, begin:end]
                            )
                        )
                    )
                else:
                    ...
                    active_logits_finite.append(
                        bool(np.all(np.isfinite(logits[request])))
                    )
            ...
            item = {
                "case": case_name,
                "logits_vs_b4_eager": logits_metric,
                "key_cache_vs_b4_eager": key_metric,
                "value_cache_vs_b4_eager": value_metric,
                "next_position_equal": bool(
                    np.array_equal(position, expected_position)
                ),
                ...
                "unaddressed_value_elementwise_exact": bool(
                    np.array_equal(
                        flat_value[:, ~mutable], flat_input_value[:, ~mutable]
                    )
                ),
    ...
    result = {
        "gate": "G4c Attempt 69d B=4 native GE versus batched eager",
        ...
        "reference_archive_key_note": (
            "Attempt 69b's historical _b2_ key spelling contains true B=4 tensors."
        ),
        "cases": cases,
        "claim_boundary": (
            "This closes B=4 single-step native GE semantics only. The B=4 "
            "resident epoch, performance, recovery, and vLLM integration remain open."
        ),
    }

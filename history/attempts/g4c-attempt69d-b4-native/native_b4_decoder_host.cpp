...
namespace {
constexpr size_t kTokenBytes = 4ULL * sizeof(int64_t);
constexpr size_t kPositionBytes = 4ULL * sizeof(int64_t);
constexpr size_t kSequenceLengthBytes = 4ULL * sizeof(int32_t);
constexpr size_t kCacheBytes =
    28ULL * 8ULL * 128ULL * 4ULL * 128ULL * sizeof(uint16_t);
constexpr size_t kSlotMappingBytes = 4ULL * sizeof(int32_t);
constexpr size_t kActiveMaskBytes = 4ULL * sizeof(int32_t);
constexpr size_t kBlockTableBytes = 8ULL * sizeof(int32_t);
constexpr size_t kTilingBytes = 72ULL;
constexpr size_t kLogitsBytes = 4ULL * 152064ULL * sizeof(float);
...
const std::array<InputSpec, 9> kInputs = {{
    {"token_id.bin", {4, 1}, ge::DT_INT64, kTokenBytes},
    {"position.bin", {4}, ge::DT_INT64, kPositionBytes},
    {"sequence_length.bin", {4, 1}, ge::DT_INT32, kSequenceLengthBytes},
    {"key_cache.bin", {28, 8, 128, 4, 128}, ge::DT_BF16, kCacheBytes},
    {"slot_mapping.bin", {4}, ge::DT_INT32, kSlotMappingBytes},
    {"active_mask.bin", {4}, ge::DT_INT32, kActiveMaskBytes},
    {"block_table.bin", {4, 2}, ge::DT_INT32, kBlockTableBytes},
    {"value_cache.bin", {28, 8, 128, 4, 128}, ge::DT_BF16, kCacheBytes},
...
int RunCase(const std::shared_ptr<ge::Session> &session,
  ...
  std::cout << "G4C_ATTEMPT69D_CASE case=" << case_index
            << " outputs=" << outputs.size() << std::endl;
  return 0;
}
}  // namespace
...
int main(int argc, char **argv) {
  if (argc != 4) {
    std::cerr << "usage: native_b4_decoder_host AIR INPUT_ROOT OUTPUT_ROOT\n";
    return 2;
  }
  ge::Graph graph("G4cAttempt69dB4Decoder");
  const auto load_status = graph.LoadFromFile(argv[1]);
  std::cout << "G4C_ATTEMPT69D_LOAD status=" << load_status
  ...
  session.reset();
  ge::GEFinalize();
  std::cout << "G4C_ATTEMPT69D_NATIVE_COMPLETE" << std::endl;
  return 0;
}

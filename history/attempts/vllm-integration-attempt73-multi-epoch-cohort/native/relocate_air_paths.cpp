...
int main(int argc, char **argv) {
  ...
  ge::Graph source("Attempt71RuntimeRelocationSource");
  ...
  for (const auto &node : source.GetAllNodes()) {
    ...
    if (!StartsWith(old_path, old_prefix + "/")) return 7;
    ...
    if (node.SetAttr(file_path_attr, updated) != ge::GRAPH_SUCCESS) {
      return 10;
    }
    ++rewritten;
  }
  ...
  ge::Graph reloaded("Attempt71RuntimeRelocationReloaded");
  ...
  const bool paths_valid =
      AuditPaths(reloaded, new_prefix, old_prefix, audited);
  const bool pass = signature_equal && paths_valid &&
  ...
  if (!report) return 15;
  report << "{\n"
         << "  \"gate\": \"Attempt 71 runtime AIR relocation\",\n"
         ...
         << "}\n";
  report.close();
  std::cout << "ATTEMPT71_AIR_RELOCATION rewritten=" << rewritten
            << " audited=" << audited

...
src=${root}/attempt73-src
evidence=${root}/evidence-attempt73r2-multi-epoch-cohort
scratch=/dev/shm/a73r2
...
runtime_air=${scratch}/qwen_b4_decoder_step_attempt73r2.air
cache=${scratch}/cache
...
if [[ ! -f "${guard}" || ! -f "${frozen_air}" || ! -f "${tiling}" ||
      ...
      ! -f "${src}/run_multi_epoch_cohort.py" ||
      ! -f "${src}/verify_multi_epoch_cohort_result.py" ||
...
step_status=$?
[[ ${step_status} -eq 0 ]] || exit ${step_status}
...
run_step relocate-runtime-air 600s "${build}/relocate_air_paths" \
  ...
  "${runtime_export}" "${evidence}/attempt73r2-air-relocation.json"
step_status=$?
[[ ${step_status} -eq 0 ]] || exit ${step_status}
sha256sum "${runtime_air}" >"${evidence}/runtime-air-integrity.log"

wait_npu_ready pre-native || exit $?
run_step multi-epoch-native 10800s python3 "${src}/run_multi_epoch_cohort.py" \
  --model-config "${model_config}" \
  --baseline-result "${baseline}" \
  --output "${evidence}/attempt73r2-multi-epoch-result.json"
step_status=$?
[[ ${step_status} -eq 0 ]] || exit ${step_status}

run_step multi-epoch-result-verify 120s python3 \
  "${src}/verify_multi_epoch_cohort_result.py" \
  "${evidence}/attempt73r2-multi-epoch-result.json"
step_status=$?
[[ ${step_status} -eq 0 ]] || exit ${step_status}
...
idle_status=$?
capture_after
sha256sum "${evidence}/attempt73r2-multi-epoch-result.json" "${status}" \
  "${evidence}/native-bridge-integrity.log" \
...
finalized=1
cat "${evidence}/attempt73r2-multi-epoch-result.json"
trap - EXIT

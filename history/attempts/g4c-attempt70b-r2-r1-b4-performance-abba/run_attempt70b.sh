...
src=${root}/attempt70b-r2-r1-src
evidence=${root}/evidence-attempt70b-r2-r1-b4-performance
scratch=/dev/shm/ascend-control-g4-20260726/attempt70b-r2-r1-b4-performance
...
run_step analyze-abba-performance 1800s python3 \
  ...
  --output "${evidence}/attempt70b-r2-r1-result.json"
analysis_status=$?
...
idle_status=$?
capture_after
sha256sum "${evidence}/attempt70b-r2-r1-result.json" "${status}" \
  >"${evidence}/result-integrity.log" 2>/dev/null || true
storage_guard_finalize
finalize_status=$?
[[ ${finalize_status} -eq 0 ]] || exit ${finalize_status}
finalized=1
cat "${evidence}/attempt70b-r2-r1-result.json" 2>/dev/null ||

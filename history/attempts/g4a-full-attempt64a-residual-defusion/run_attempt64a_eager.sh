...
python "${src}/compare_eager_subset.py" --baseline "${baseline}" \
  --candidate "${raw}/attempt64a-eager-reference.npz" \
...
python "${src}/compare_eager.py" --baseline "${baseline_eager}" \
  --candidate "${raw}/attempt64a-eager-reference.npz" \
  --output "${raw}/eager-attempt62a-comparison.json" \
...
sha256sum "${raw}/attempt64a-eager-reference.npz" "${raw}/eager-screen.json" \
  "${raw}/eager-comparison.json" "${raw}/eager-attempt62a-comparison.json" \
  >"${raw}/result-integrity.log" 2>/dev/null || true
cat "${raw}/eager-screen.json"
cat "${raw}/eager-comparison.json"
cat "${raw}/eager-attempt62a-comparison.json"

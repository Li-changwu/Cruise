...
src=${root}/attempt53k-native-src
export_dir=${root}/export-attempt53k
raw=${root}/raw-attempt53k-native
cache=${root}/cache-attempt53k-native
...
g++ -D_GLIBCXX_USE_CXX11_ABI=0 -O2 -std=c++11 -ftrapv -fstack-protector-all \
  ...
  -o "${raw}/native_full_decoder_attempt53k_host" >"${raw}/compile.log" 2>&1
s=$?
printf 'compile\t%s\n' "$s" >>"${status}"
[[ $s -eq 0 ]] || exit $s

timeout 3600 "${raw}/native_full_decoder_attempt53k_host" "${air}" "${inputs}" \
...
python "${src}/compare_native.py" --native-dir "${outputs}" --reference "${reference}" \
  --initial-input-dir "${inputs}" --output-npz "${raw}/attempt53k-native-output.npz" \
  --output "${raw}/attempt53k-native-result.json" >"${raw}/compare.stdout.log" 2>&1
s=$?
printf 'compare\t%s\n' "$s" >>"${status}"
sha256sum "${air}" "${raw}/attempt53k-native-output.npz" \
  "${raw}/attempt53k-native-result.json" >"${raw}/result-integrity.log" 2>/dev/null || true
...
cat "${raw}/attempt53k-native-result.json" 2>/dev/null || true
exit $s

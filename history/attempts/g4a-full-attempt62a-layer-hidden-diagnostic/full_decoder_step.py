...
def eager_screen(
    ...
    reference_path = output_dir / "attempt62a-eager-reference.npz"
    np.savez(reference_path, **reference)
    result = {
        "gate": "G4a Attempt 62a per-layer hidden diagnostic eager screen",
    ...
    print("G4A_ATTEMPT62A_EAGER " + json.dumps(result, ensure_ascii=True), flush=True)
    if not result["pass"]:
        raise SystemExit(1)
    return result
...
def export_air(
    ...
    dynamo_export(
        ...
        export_name="qwen_full_decoder_step_attempt62a",
        dynamic=False,
        config=config,
    )
    air = output_dir / "qwen_full_decoder_step_attempt62a.air"
    graph = output_dir / "dynamo.pbtxt"
    external_files = [path for path in output_dir.iterdir() if path.is_file() and path.suffix == ""]
    result = {
        "gate": "G4a Attempt 62a per-layer hidden diagnostic AIR export",
    ...
    print("G4A_ATTEMPT62A_EXPORT " + json.dumps(result, ensure_ascii=True), flush=True)

...
src=${root}/attempt62a-src
eager=${root}/raw-attempt62a-eager
raw=${root}/raw-attempt62a-export
export_dir=${root}/export-attempt62a
cache=${root}/cache-attempt62a-export
...
sha256sum "${src}"/*.py "${src}"/*.md "${src}"/*.sh \
  "${eager}/attempt62a-eager-reference.npz" "${baseline}" \
...
timeout 10800 python "${src}/full_decoder_step.py" --mode export --model-dir "${model}" \
  ...
  --reference "${eager}/attempt62a-eager-reference.npz" \
...
python "${src}/compare_eager_subset.py" --baseline "${baseline}" \
  --candidate "${eager}/attempt62a-eager-reference.npz" \
...
sha256sum "${export_dir}/qwen_full_decoder_step_attempt62a.air" \

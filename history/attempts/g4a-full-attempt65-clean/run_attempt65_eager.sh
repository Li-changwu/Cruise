...
python "${src}/compare_eager.py" --baseline "${baseline}" \
  --candidate "${raw}/attempt65-eager-reference.npz" \
...
sha256sum "${raw}/attempt65-eager-reference.npz" "${raw}/eager-screen.json" \
  "${raw}/eager-comparison.json" \

...
src=${root}/attempt65-src
eager=${root}/raw-attempt65-eager
raw=${root}/raw-attempt65-export
export_dir=${root}/export-attempt65
cache=${root}/cache-attempt65-export
...
sha256sum "${src}"/*.py "${src}"/*.md "${src}"/*.sh \
  "${eager}/attempt65-eager-reference.npz" "${baseline}" \
...
timeout 10800 python "${src}/full_decoder_step.py" --mode export --model-dir "${model}" \
  ...
  --reference "${eager}/attempt65-eager-reference.npz" \
...
python "${src}/compare_eager.py" --baseline "${baseline}" \
  --candidate "${eager}/attempt65-eager-reference.npz" \
...
sha256sum "${export_dir}/qwen_full_decoder_step_attempt65.air" \

...
class PagedQwenDecoderStep(torch.nn.Module):
    ...
    def forward(
        ...
        for layer_index, layer in enumerate(self.layers):
            ...
            hidden = residual + F.linear(attention, layer.o_weight)
            ...
            hidden = residual + F.linear(gate * up, layer.down_weight)
...
def eager_screen(
    ...
    with torch.no_grad():
        for step, token_id in enumerate(token_ids, start=1):
            ...
            logits, key, value, next_position, layer_hidden = model(
            ...
            layer_metrics = [
                tensor_metrics(
                    (layer_hidden_bits[index].astype(np.uint32) << 16).view(np.float32),
                    (
                        official[step - 1]["layer_hidden_bits"][index].astype(np.uint32)
                        << 16
                    ).view(np.float32),
                )
                for index in range(NUM_LAYERS)
    ...
    reference_path = output_dir / "attempt53h-eager-reference.npz"
    np.savez(reference_path, **reference)
    result = {
        "gate": "G4a Attempt 53h complete-decoder per-layer localization",

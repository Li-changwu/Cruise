...
def main() -> None:
    ...
    parser.add_argument("--expected", type=int, default=452)
    ...
    with args.log.open("r", encoding="utf-8", errors="replace") as stream:
        for line in stream:
            start = re.search(r"Distribute Start, op: (LinearTransposeX2[^, ]*)", line)
            if start:
            ...
            if f"Distribute Success, op: {active_op}" in line:
                active_op = None
    ...
    valid = (
        ...
        and len(op_counts) == 113
        and set(op_counts.values()) == {4}
    ...
    result = {
        ...
        "all_ops_launched_four_times": bool(op_counts) and set(op_counts.values()) == {4},

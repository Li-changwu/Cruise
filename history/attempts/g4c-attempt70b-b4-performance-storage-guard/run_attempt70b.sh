...
src=${root}/attempt70b-src
evidence=${root}/evidence-attempt70b-b4-performance
scratch=/dev/shm/ascend-control-g4-20260726/attempt70b-b4-performance
...
prepared_runner=${G4_PREPARED_RUNNER_DIR:-/dev/shm/ascend-control-g4-20260726/attempt70b-prepared-runner}
raw=${scratch}/raw
cache=${scratch}/cache
build=${scratch}/build
...
sha256sum "${build}/g4c_b4_epoch_runner" >"${evidence}/performance-runner-integrity.log"
...
if storage_guard_wait_for_npu_idle 7 3600 "${root}" 24 \
  ...
  printf '%s\n' "${STORAGE_GUARD_WAITED_NPU_USAGE}" \
    >"${evidence}/npu7-usages-pre-device.txt"
...
run_step performance-suite 10800s "${build}/g4c_b4_epoch_runner" perf "${air}" \
  "${src}/config/graph_config.json" \
  "${src}/config/g4c_b4_resident_epoch_func.json" \
  "${inputs}" "${perf_outputs}" 0 0 0 0
performance_status=$?
if [[ ${performance_status} -ne 0 ]]; then
  tail -320 "${evidence}/performance-suite.stdout.log"
  exit ${performance_status}
fi

run_step analyze-performance 1800s python3 "${src}/analyze_performance.py" \
  --input "${perf_outputs}/perf.tsv" \
  --output "${evidence}/attempt70b-result.json"
analysis_status=$?

find "${inputs}" "${perf_outputs}" -type f -print0 |
...
sha256sum "${evidence}/attempt70b-result.json" "${status}" \
  >"${evidence}/result-integrity.log" 2>/dev/null || true
storage_guard_finalize
finalize_status=$?
[[ ${finalize_status} -eq 0 ]] || exit ${finalize_status}
finalized=1
cat "${evidence}/attempt70b-result.json" 2>/dev/null || \
  tail -240 "${evidence}/analyze-performance.stdout.log"
trap - EXIT

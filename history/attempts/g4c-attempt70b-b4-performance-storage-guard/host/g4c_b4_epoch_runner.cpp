...
int RunCombinedPerf(const std::string &air_path,
                    const std::string &graph_config,
                    const std::string &func_config,
                    const std::string &input_root,
                    const std::string &output_root) {
  constexpr int32_t kWarmups = 3;
  constexpr int32_t kRepeats = 15;
  ...
  ge::Graph host_graph("Attempt70bB4HostPerf");
  const auto load_status = host_graph.LoadFromFile(air_path.c_str());
  if (load_status != ge::GRAPH_SUCCESS || !host_graph.IsValid()) return 61;
  auto flow_graph = BuildDeviceFlow(air_path, graph_config, func_config);
  std::map<ge::AscendString, ge::AscendString> options = {
      {"ge.exec.deviceId", "0"},
      {"ge.exec.logicalDeviceClusterDeployMode", "SINGLE"},
      ...
      {"ge.graphRunMode", "0"}};
  auto ret = ge::GEInitialize(options);
  if (ret != ge::SUCCESS) return 62;
  auto host_session = std::make_shared<ge::Session>(
      std::map<ge::AscendString, ge::AscendString>{});
  auto device_session = std::make_shared<ge::Session>(
      std::map<ge::AscendString, ge::AscendString>{});
  ret = host_session->AddGraph(0, host_graph);
  if (ret == ge::SUCCESS) {
    ret = device_session->AddGraph(0, flow_graph.ToGeGraph());
  }
  if (ret != ge::SUCCESS) {
  ...
  std::ofstream stream(output_root + "/perf.tsv", std::ios::trunc);
  if (!stream) {
    device_session.reset();
    host_session.reset();
    ge::GEFinalize();
  ...
  stream << "phase\tk\titeration\torder\tposition\troute\twall_us\tcpu_us"
            "\thost_model_submissions\tfeed_calls\tfetch_calls\n";

  auto run_pair = [&](const char *phase, size_t case_index, int32_t iteration,
                      bool host_first) -> int {
    const char *order = host_first ? "HD" : "DH";
    auto run_route = [&](bool host, int32_t position) -> int {
      Timing timing;
      int status = 0;
      if (host) {
        status = RunHostCase(host_session, bases[case_index], "",
                             steps[case_index], eos, nullptr, &timing, false);
      } else {
        status = RunDeviceCase(device_session, bases[case_index], "",
                               steps[case_index], eos, &timing, false);
      }
      if (status != 0) return status;
      stream << phase << '\t' << steps[case_index] << '\t' << iteration << '\t'
             << order << '\t' << position << '\t'
    ...
    int status = run_route(host_first, 0);
    if (status != 0) return status;
    return run_route(!host_first, 1);
  };
  ...
  for (size_t case_index = 0; case_index < steps.size() && status == 0;
    ...
    for (int32_t iteration = 0; iteration < kWarmups && status == 0;
         ++iteration) {
      status = run_pair("warmup", case_index, iteration,
                        iteration % 2 == 0);
    }
    for (int32_t iteration = 0; iteration < kRepeats && status == 0;
         ++iteration) {
      status = run_pair("measure", case_index, iteration,
                        iteration % 2 == 0);
    }
  }
  ...
  ge::GEFinalize();
  if (status != 0) return status;
  std::cout << "B4_PERF_COMPLETE warmups=" << kWarmups
            << " repeats=" << kRepeats << std::endl;
  return 0;
}
}  // namespace
...
int main(int argc, char **argv) {
  ...
  if (route == "perf") {
    return RunCombinedPerf(argv[2], argv[3], argv[4], argv[5], argv[6]);
  }
  if (route == "device") {

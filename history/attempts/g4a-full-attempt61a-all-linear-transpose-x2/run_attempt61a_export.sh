...
src=${root}/attempt61a-src
eager=${root}/raw-attempt61a-eager
raw=${root}/raw-attempt61a-export
export_dir=${root}/export-attempt61a
cache=${root}/cache-attempt61a-export
...
sha256sum "${src}"/*.py "${src}"/*.md "${src}"/*.sh \
  "${eager}/attempt61a-eager-reference.npz" "${baseline}" \
...
timeout 10800 python "${src}/full_decoder_step.py" --mode export --model-dir "${model}" \
  ...
  --reference "${eager}/attempt61a-eager-reference.npz" \
...
python "${src}/compare_eager.py" --baseline "${baseline}" \
  --candidate "${eager}/attempt61a-eager-reference.npz" \
...
sha256sum "${export_dir}/qwen_full_decoder_step_attempt61a.air" \

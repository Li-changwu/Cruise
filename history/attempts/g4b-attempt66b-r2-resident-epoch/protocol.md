# G4b Attempt 66b-r2: B=1 device-resident real generation epoch
...
cast from `const uint8_t*` with `static_cast`. Attempt 66b-r2 changes only that
conversion to `reinterpret_cast<const float*>`; no graph, control, input,
tolerance or comparison semantics change.

...
src=${root}/attempt66b-r2-src
raw=${root}/raw-attempt66b-r2
cache=${root}/cache-attempt66b-r2
build=${root}/build-attempt66b-r2
...
python "${src}/compare_epochs.py" --host-dir "${host_outputs}" \
  ...
  --eager-reference "${reference}" --output "${raw}/attempt66b-r2-result.json" \
...
sha256sum "${raw}/attempt66b-r2-result.json" \
  >"${raw}/result-integrity.log" 2>/dev/null || true
cat "${raw}/attempt66b-r2-result.json" 2>/dev/null || \

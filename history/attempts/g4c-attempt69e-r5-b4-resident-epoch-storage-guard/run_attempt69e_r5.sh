...
src=${root}/attempt69e-r5-src
evidence=${root}/evidence-attempt69e-r5-b4-resident-epoch
scratch=/dev/shm/ascend-control-g4-20260726/attempt69e-r5-b4-resident-epoch
prepared=${G4_PREPARED_DIR:-/dev/shm/ascend-control-g4-20260726/attempt69e-r4-b4-resident-epoch}
prepared_evidence=${root}/evidence-attempt69e-r4-b4-resident-epoch
prepared_source=${root}/attempt69e-r4-src
raw=${scratch}/raw
cache=${scratch}/cache
build=${scratch}/build
...
mkdir -p "${raw}" "${cache}" "${weights}" "${cann_logs}" \
...
reuse_host=0
prepared_stage_ok=0
if [[ -f "${prepared_evidence}/status.tsv" ]] &&
   { { grep -Fqx $'prepare-inputs\t0' "${prepared_evidence}/status.tsv" &&
       grep -Fqx $'build\t0' "${prepared_evidence}/status.tsv"; } ||
     { grep -Fqx $'reuse-prepared\t0' "${prepared_evidence}/status.tsv" &&
       grep -Fqx $'reuse-prepared-integrity\t0' "${prepared_evidence}/status.tsv"; }; }; then
  prepared_stage_ok=1
fi
if storage_guard_require_child "${prepared}" /dev/shm &&
   [[ -f "${prepared}/.storage-guard-scratch.tsv" &&
      -x "${prepared}/build/g4c_b4_epoch_runner" &&
      -d "${prepared}/raw/inputs" &&
      ${prepared_stage_ok} -eq 1 ]] &&
   cmp -s "${prepared_source}/host/CMakeLists.txt" "${src}/host/CMakeLists.txt" &&
   cmp -s "${prepared_source}/host/g4c_b4_epoch_runner.cpp" \
     "${src}/host/g4c_b4_epoch_runner.cpp"; then
  reuse_prepared=1
fi
if (( reuse_prepared == 1 )) &&
   grep -Fqx $'host-epoch-suite\t0' "${prepared_evidence}/status.tsv" &&
   [[ -f "${prepared}/raw/host/early_eos_tokens.txt" ]]; then
  reuse_host=1
fi
...
if (( reuse_prepared == 1 )); then
  run_step reuse-prepared 900s bash -c '
    ...
    cp -a --reflink=auto "$1/build" "$2/build"
    if [[ "$3" == 1 ]]; then
      cp -a --reflink=auto "$1/raw/host/." "$2/raw/host/"
    fi
  ' _ "${prepared}" "${scratch}" "${reuse_host}"
  reuse_status=$?
  ...
  source_build_hash=$(sha256sum "${prepared}/build/g4c_b4_epoch_runner" | awk '{print $1}')
  copied_build_hash=$(sha256sum "${build}/g4c_b4_epoch_runner" | awk '{print $1}')
  if [[ ${source_input_bytes} -ne ${copied_input_bytes} ||
        ${source_input_files} -ne ${copied_input_files} ||
        "${source_build_hash}" != "${copied_build_hash}" ]]; then
    printf 'reuse-prepared-integrity\t93\n' >>"${status}"
    exit 93
  fi
  if (( reuse_host == 1 )); then
    source_host_bytes=$(storage_guard_used_bytes "${prepared}/raw/host")
    copied_host_bytes=$(storage_guard_used_bytes "${host_outputs}")
    source_host_files=$(find "${prepared}/raw/host" -type f | wc -l)
    copied_host_files=$(find "${host_outputs}" -type f | wc -l)
    if [[ ${source_host_bytes} -ne ${copied_host_bytes} ||
          ${source_host_files} -ne ${copied_host_files} ]]; then
      printf 'reuse-host-integrity\t93\n' >>"${status}"
      exit 93
    fi
    printf 'reuse-host-integrity\t0\n' >>"${status}"
  fi
  printf 'reuse-prepared-integrity\t0\n' >>"${status}"
  {
    printf 'prepared_dir\t%s\n' "$(storage_guard_realpath "${prepared}")"
    printf 'prepared_evidence\t%s\n' "${prepared_evidence}"
    printf 'source_input_bytes\t%s\n' "${source_input_bytes}"
    printf 'copied_input_bytes\t%s\n' "${copied_input_bytes}"
    printf 'source_input_files\t%s\n' "${source_input_files}"
    printf 'copied_input_files\t%s\n' "${copied_input_files}"
    printf 'source_build_sha256\t%s\n' "${source_build_hash}"
    printf 'copied_build_sha256\t%s\n' "${copied_build_hash}"
    printf 'reuse_host\t%s\n' "${reuse_host}"
    if (( reuse_host == 1 )); then
      printf 'source_host_bytes\t%s\n' "${source_host_bytes}"
      printf 'copied_host_bytes\t%s\n' "${copied_host_bytes}"
      printf 'source_host_files\t%s\n' "${source_host_files}"
      printf 'copied_host_files\t%s\n' "${copied_host_files}"
    fi
  } >"${evidence}/prepared-reuse.tsv"
else
...
if storage_guard_wait_for_npu_idle 7 3600 "${root}" 32 \
...
run_step compare 1800s python3 "${src}/compare_epochs.py" \
  ...
  --output "${evidence}/attempt69e-r5-result.json"
compare_status=$?
...
sha256sum "${evidence}/attempt69e-r5-result.json" "${status}" \
  >"${evidence}/result-integrity.log" 2>/dev/null || true
storage_guard_finalize
finalize_status=$?
[[ ${finalize_status} -eq 0 ]] || exit ${finalize_status}
finalized=1
cat "${evidence}/attempt69e-r5-result.json" 2>/dev/null || \

...
class Layer0BoundaryProbe(torch.nn.Module):
    ...
    def forward(
        ...
        gate_input = post_attention_norm.reshape(-1, HIDDEN_SIZE)
        gate_weight = self.gate_weight.transpose(0, 1)
        gate_preactivation = torch.ops.g4a_materialize.materialize(
            torch.ops.g4a_matmulv2.mm(gate_input, gate_weight).reshape(
                1, 1, INTERMEDIATE_SIZE
            )
        )
        gate = F.silu(gate_preactivation)
        up = F.linear(post_attention_norm, self.up_weight)
        mlp_product = gate * up
...
def register_custom_ops(
    ...
    import gate_matmulv2  # noqa: F401
...
def main() -> None:
    ...
    reference_path = args.output_dir / "attempt58a-eager-reference.npz"
    ...
    dynamo_export(
        ...
        export_name="qwen_layer0_boundary_attempt58a",
        dynamic=False,
        config=config,
    )
    air = args.output_dir / "qwen_layer0_boundary_attempt58a.air"

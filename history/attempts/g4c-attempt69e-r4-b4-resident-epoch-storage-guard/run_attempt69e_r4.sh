...
src=${root}/attempt69e-r4-src
evidence=${root}/evidence-attempt69e-r4-b4-resident-epoch
scratch=/dev/shm/ascend-control-g4-20260726/attempt69e-r4-b4-resident-epoch
...
run_step compare 1800s python3 "${src}/compare_epochs.py" \
  ...
  --output "${evidence}/attempt69e-r4-result.json"
compare_status=$?
...
sha256sum "${evidence}/attempt69e-r4-result.json" "${status}" \
  >"${evidence}/result-integrity.log" 2>/dev/null || true
storage_guard_finalize
finalize_status=$?
[[ ${finalize_status} -eq 0 ]] || exit ${finalize_status}
finalized=1
cat "${evidence}/attempt69e-r4-result.json" 2>/dev/null || \

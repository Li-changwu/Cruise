# G4a Attempt 53e: Native-QK Complete-Decoder Eager Control

Date frozen: 2026-07-23

## Claim

This controlled eager experiment tests whether Attempt 53d's step-4 logits
drift comes from the complete Paged-KV decoder semantics or from ExactQk.
...
- The only computational change from Attempt 53d is replacing `ExactQk` with
  native `torch.matmul`; FP32 scale, BF16 cast and `Bf16Barrier` are unchanged.

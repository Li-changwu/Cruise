...
src=${root}/attempt55a-src
raw=${root}/raw-attempt55a-export
export_dir=${root}/export-attempt55a
cache=${root}/cache-attempt55a-export
...
timeout 3600 python "${src}/layer0_boundary_probe.py" --model-dir "${model}" \
  ...
  --barrier-source "${barrier}" --output-dir "${export_dir}" \
...
if [[ ${exact_count} -ne 1 || ${barrier_count} -ne 1 ]]; then
...
sha256sum "${export_dir}/qwen_layer0_boundary_attempt55a.air" \
  "${export_dir}/dynamo.pbtxt" "${export_dir}/attempt55a-eager-reference.npz" \

...
src=${root}/attempt55d-src
raw=${root}/raw-attempt55d-export
export_dir=${root}/export-attempt55d
cache=${root}/cache-attempt55d-export
...
if [[ ${exact_count} -ne 1 || ${barrier_count} -ne 1 || ${materialize_count} -ne 2 ]]; then
...
sha256sum "${export_dir}/qwen_layer0_boundary_attempt55d.air" \
  "${export_dir}/dynamo.pbtxt" "${export_dir}/attempt55d-eager-reference.npz" \

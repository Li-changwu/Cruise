# G4a Attempt 53k Native2 Complete-Decoder Gate
...
This native2 attempt keeps only the legacy GE `deviceId` and `graphRunMode`
initialization used by the validated attention runners. It deliberately leaves
`RESOURCE_CONFIG_PATH` unset and does not enable the GE op compiler cache; these
are the only runtime-config changes from native1 after its external-weight
model-copy failure.

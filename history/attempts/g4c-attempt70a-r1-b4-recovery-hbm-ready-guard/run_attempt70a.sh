...
src=${root}/attempt70a-r1-src
evidence=${root}/evidence-attempt70a-r1-b4-recovery
scratch=/dev/shm/ascend-control-g4-20260726/attempt70a-r1-b4-recovery
...
run_step compare-recovery 1800s python3 "${src}/compare_recovery.py" \
  ...
  --output "${evidence}/attempt70a-r1-result.json"
compare_status=$?
...
sha256sum "${evidence}/attempt70a-r1-result.json" "${status}" \
  >"${evidence}/result-integrity.log" 2>/dev/null || true
storage_guard_finalize
finalize_status=$?
[[ ${finalize_status} -eq 0 ]] || exit ${finalize_status}
finalized=1
cat "${evidence}/attempt70a-r1-result.json" 2>/dev/null || \

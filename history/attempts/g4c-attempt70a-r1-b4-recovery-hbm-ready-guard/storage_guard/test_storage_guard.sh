...
fake_high_hbm_npu=${test_root}/fake-high-hbm-npu-smi
...
cat >"${fake_high_hbm_npu}" <<'EOF'
#!/usr/bin/env bash
printf '\tNo process in device.\n'
printf '\tHBM Usage Rate(%%)              : 63\n'
EOF
chmod 700 "${fake_high_hbm_npu}"
...
export STORAGE_GUARD_NPU_SMI=${fake_high_hbm_npu}
if storage_guard_preflight "${persistent}" "${persistent}/high-hbm" \
  "${scratch_parent}/high-hbm" 99 0 1 0; then
  exit 1931
elif [[ $? -ne 95 ]]; then
  exit 1932
fi
[[ ! -e "${persistent}/high-hbm" && ! -e "${scratch_parent}/high-hbm" ]] || exit 1933

...
int RunDeviceCase(const std::shared_ptr<ge::Session> &session,
                  ...
                  const std::string &output_dir, int32_t max_steps,
                  const std::array<int32_t, kBatchSize> &eos,
                  int32_t sampling_mode = 0, int32_t graph_variant = 0) {
  auto buffers = base;
  ...
  control_values[1 + kBatchSize] = sampling_mode;
  control_values[2 + kBatchSize] = graph_variant;
...
int RunRecoverySuite(const std::string &air_path,
                     const std::string &graph_config,
                     const std::string &func_config,
                     const std::string &input_root,
                     const std::string &output_root) {
  ...
  std::array<std::vector<uint8_t>, 9> base;
  if (!LoadInputs(input_root + "/k8-all-active", base)) {
    session.reset();
    ge::GEFinalize();
    return 52;
  }
  ...
  int status = RunDeviceCase(session, base,
                             output_root + "/invalid-max-steps", 9, eos);
  if (status == 0) {
    status = RunDeviceCase(session, base,
                           output_root + "/unsupported-sampling", 1, eos, 1, 0);
  }
  if (status == 0) {
    status = RunDeviceCase(session, base,
                           output_root + "/unsupported-graph", 1, eos, 0, 1);
  }
  if (status == 0) {
    auto capacity = base;
    std::array<int64_t, kBatchSize> positions{};
    std::array<int32_t, kBatchSize> lengths{};
    std::array<int32_t, kBatchSize> slots{};
    std::array<int32_t, kBatchSize * kBlocksPerRequest> block_table{};
    std::memcpy(positions.data(), capacity[1].data(), capacity[1].size());
    std::memcpy(lengths.data(), capacity[2].data(), capacity[2].size());
    std::memcpy(slots.data(), capacity[4].data(), capacity[4].size());
    std::memcpy(block_table.data(), capacity[6].data(), capacity[6].size());
    positions[0] = kLogicalCapacity - 1;
    lengths[0] = kLogicalCapacity;
    slots[0] = block_table[0] * kBlockSize + kLogicalCapacity - 1;
    std::memcpy(capacity[1].data(), positions.data(), capacity[1].size());
    std::memcpy(capacity[2].data(), lengths.data(), capacity[2].size());
    std::memcpy(capacity[4].data(), slots.data(), capacity[4].size());
    status = RunDeviceCase(session, capacity,
                           output_root + "/capacity-exceeded", 2, eos);
  }

  session.reset();
  ge::GEFinalize();
  if (status != 0) return status;
  std::cout << "DEVICE_B4_RECOVERY_SUITE_COMPLETE cases=4" << std::endl;
  return 0;
}
}  // namespace
...
int main(int argc, char **argv) {
  ...
  if (route == "recovery") {
    return RunRecoverySuite(argv[2], argv[3], argv[4], argv[5], argv[6]);
  }
  if (route == "device") {

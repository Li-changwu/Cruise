...
class PagedQwenDecoderStep(torch.nn.Module):
    ...
    def rope(self, position: torch.Tensor, dtype: torch.dtype) -> tuple[torch.Tensor, torch.Tensor]:
        frequencies = position.float().reshape(self.batch_size, 1) * self.inv_freq.view(
            1, HEAD_DIM // 2
        )
    ...
    def update_and_read_cache(
        ...
        flat = layer_cache.reshape(
            self.physical_blocks * BLOCK_SIZE, NUM_KV_HEADS, HEAD_DIM
        )
        updated_flat = flat
        for batch_index in range(self.batch_size):
            slot_mask = (
                self.physical_slots == slot_mapping[batch_index : batch_index + 1]
            ).view(-1, 1, 1)
            slot_mask = slot_mask & active_mask[
                batch_index : batch_index + 1
            ].to(torch.bool).view(1, 1, 1)
            replacement = new_value[batch_index].reshape(
                1, NUM_KV_HEADS, HEAD_DIM
            )
            updated_flat = torch.where(slot_mask, replacement, updated_flat)
        ...
        for batch_index in range(self.batch_size):
            physical_blocks = torch.index_select(
                block_table[batch_index], 0, logical_blocks.to(torch.int64)
            )
            dense_slots = (physical_blocks * BLOCK_SIZE + offsets).to(torch.int64)
            dense_batches.append(torch.index_select(updated_flat, 0, dense_slots))
        dense = torch.stack(dense_batches)
        return updated_flat.reshape_as(layer_cache), dense.permute(0, 2, 1, 3).contiguous()
    ...
    def forward(
        ...
        for layer_index, layer in enumerate(self.layers):
            ...
            expanded_key = (
                ...
                .reshape(self.batch_size, NUM_HEADS, LOGICAL_CAPACITY, HEAD_DIM)
            )
            expanded_value = (
                dense_value.unsqueeze(2)
                .expand(
                    self.batch_size,
                    NUM_KV_HEADS,
                    NUM_KV_GROUPS,
            ...
            for batch_index in range(self.batch_size):
                qk_batches.append(
                    torch.ops.g4a_qk.exact_qk(
                        query[batch_index : batch_index + 1]
                        .squeeze(2)
                        .contiguous(),
                        expanded_key[batch_index].transpose(1, 2).contiguous(),
                        explicit_tiling,
                    )
                )
            raw_qk = torch.cat(qk_batches, dim=0)
            ...
            scores = torch.where(
                valid.view(self.batch_size, 1, 1, LOGICAL_CAPACITY),
                scores,
            ...
            ).reshape(self.batch_size, 1, INTERMEDIATE_SIZE)
        ...
        ).reshape(self.batch_size, 1, VOCAB_SIZE).float()
        next_position = torch.where(
            active_mask.to(torch.bool), position + 1, position
        )
        return (
            logits,
...
def eager_screen(
    ...
    reference_path = output_dir / "attempt65-eager-reference.npz"
    np.savez(reference_path, **reference)
    result = {
        "gate": "G4a Attempt 65 clean residual-defused decoder eager screen",
        ...
        "steps": step_results,
        "claim_boundary": (
            "Clean four-output B=1 decoder eager prerequisite only; AIR and native GE "
            "correctness must still pass before G4a."
        ),
    }
    result_path = output_dir / "eager-screen.json"
    result_path.write_text(json.dumps(result, indent=2, ensure_ascii=True) + "\n", encoding="utf-8")
    print("G4A_ATTEMPT65_EAGER " + json.dumps(result, ensure_ascii=True), flush=True)
    if not result["pass"]:
        raise SystemExit(1)
    return result
...
def export_air(
    ...
    dynamo_export(
        ...
        export_name="qwen_full_decoder_step_attempt65",
        dynamic=False,
        config=config,
    )
    air = output_dir / "qwen_full_decoder_step_attempt65.air"
    graph = output_dir / "dynamo.pbtxt"
    external_files = [path for path in output_dir.iterdir() if path.is_file() and path.suffix == ""]
    result = {
        "gate": "G4a Attempt 65 clean residual-defused decoder AIR export",
        ...
        "reference_sha256": sha256(reference_path),
        "claim_boundary": (
            "Clean four-output AIR only; native GE recurrence must still pass before G4a."
        ),
    }
    (output_dir / "export-result.json").write_text(
        json.dumps(result, indent=2, ensure_ascii=True) + "\n", encoding="utf-8"
    )
    print("G4A_ATTEMPT65_EXPORT " + json.dumps(result, ensure_ascii=True), flush=True)

...
def main() -> None:
    ...
    for name, op, block in nodes:
        if op != "Data" or 'dtype: DT_INT32' not in block:
            continue
        if descriptor_shape(block, "[o]y") != (4,):
            continue
        consumers = []
    ...
    active_data = [
        item for item in int32_vector_data if item["direct_consumer_count"] == 225
    ]
    slot_data = [
        item for item in int32_vector_data if item["direct_consumer_count"] == 224
    ]
    result = {
        "valid": (

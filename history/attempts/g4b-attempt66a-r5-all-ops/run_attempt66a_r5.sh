...
src=${root}/attempt66a-r5-src
raw=${root}/raw-attempt66a-r5-all-ops
cache=${root}/cache-attempt66a-r5-all-ops
build=${root}/build-attempt66a-r5-all-ops
...
python "${src}/compare_outputs.py" --reference "${reference}" \
  --actual-dir "${raw}/outputs" --output "${raw}/attempt66a-r5-result.json" \
...
sha256sum "${raw}/attempt66a-r5-result.json" \
  >"${raw}/result-integrity.log" 2>/dev/null || true
cat "${raw}/attempt66a-r5-result.json" 2>/dev/null || \

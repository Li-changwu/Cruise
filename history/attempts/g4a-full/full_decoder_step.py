...
class PagedQwenDecoderStep(torch.nn.Module):
    ...
    def forward(
        ...
        for layer_index, layer in enumerate(self.layers):
            ...
            scores = (raw_qk.float() / math.sqrt(HEAD_DIM)).to(torch.bfloat16).unsqueeze(2)
            scores = torch.where(
        ...
        return logits, torch.stack(new_key_layers), torch.stack(new_value_layers), position + 1
...
def register_exact_qk(custom_source: Path) -> None:
    sys.path.insert(0, str(custom_source))
...
def eager_screen(model_dir: Path, custom_source: Path, output_dir: Path) -> dict:
    from transformers import AutoTokenizer

    register_exact_qk(custom_source)
    ...
    reference_path = output_dir / "attempt53-eager-reference.npz"
    np.savez(reference_path, **reference)
    result = {
        "gate": "G4a Attempt 53 independent complete-decoder eager screen",
...
def export_air(model_dir: Path, custom_source: Path, reference_path: Path, output_dir: Path) -> dict:
    register_exact_qk(custom_source)
    import torch_npu  # noqa: F401
    ...
    dynamo_export(
        ...
        export_name="qwen_full_decoder_step_attempt53",
        dynamic=False,
        config=config,
    )
    air = output_dir / "qwen_full_decoder_step_attempt53.air"
    graph = output_dir / "dynamo.pbtxt"
    external_files = [path for path in output_dir.iterdir() if path.is_file() and path.suffix == ""]
    result = {
        "gate": "G4a Attempt 53 complete decoder AIR export",
...
def main() -> None:
    ...
    if args.mode == "audit":
        return
    if args.custom_source is None:
        parser.error("--custom-source is required for eager/export")
    torch.npu.set_device(0)
    if args.mode == "eager":
        eager_screen(args.model_dir, args.custom_source, args.output_dir)
    else:
        if args.reference is None:
            parser.error("--reference is required for export")
        export_air(args.model_dir, args.custom_source, args.reference, args.output_dir)

...
timeout 10800 python "${src}/full_decoder_step.py" --mode export --model-dir "${model}" \
  --custom-source "${custom}" --reference "${eager}/attempt53-eager-reference.npz" \
...
sha256sum "${src}"/* "${eager}/attempt53-eager-reference.npz" \
  "${export_dir}/qwen_full_decoder_step_attempt53.air" "${export_dir}/dynamo.pbtxt" \

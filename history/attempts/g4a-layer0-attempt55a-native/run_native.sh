...
src=${root}/attempt55a-native-src
export_dir=${root}/export-attempt55a
raw=${root}/raw-attempt55a-native
cache=${root}/cache-attempt55a-native
...
g++ -D_GLIBCXX_USE_CXX11_ABI=0 -O2 -std=c++11 -ftrapv -fstack-protector-all \
  ...
  "${src}/native_layer0_attempt55a_host.cpp" -Wl,--whole-archive \
  ...
  -o "${raw}/native_layer0_attempt55a_host" >"${raw}/compile.log" 2>&1
s=$?; printf 'compile\t%s\n' "$s" >>"${status}"; [[ $s -eq 0 ]] || exit $s

timeout 1200 "${raw}/native_layer0_attempt55a_host" "${air}" "${inputs}" "${outputs}" \
...
python "${src}/compare_native.py" --native-dir "${outputs}" --reference "${reference}" \
  ...
  --output-npz "${raw}/attempt55a-native-output.npz" \
  --output "${raw}/attempt55a-result.json" >"${raw}/compare.stdout.log" 2>&1
compare_status=$?
...
sha256sum "${air}" "${raw}/attempt55a-native-output.npz" \
  "${raw}/attempt55a-result.json" >"${raw}/result-integrity.log" 2>/dev/null || true

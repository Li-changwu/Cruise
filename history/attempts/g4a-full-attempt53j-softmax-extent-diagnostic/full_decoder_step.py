...
class PagedQwenDecoderStep(torch.nn.Module):
    ...
    def forward(
        ...
        new_value_layers = []
        layer_hidden = []
        last_scaled_scores = None
        last_probabilities = None
        last_attention_projected = None
        last_after_attention = None
        last_mlp_output = None

        for layer_index, layer in enumerate(self.layers):
            residual = hidden
            normalized = self.rms_norm(hidden, layer.input_norm)
            query = F.linear(normalized, layer.q_weight, layer.q_bias)
            key = F.linear(normalized, layer.k_weight, layer.k_bias)
            value = F.linear(normalized, layer.v_weight, layer.v_bias)
            ...
            raw_qk = torch.matmul(
                query,
                expanded_key.transpose(1, 2).unsqueeze(0),
            ).squeeze(2)
            scaled_scores = (raw_qk * (HEAD_DIM**-0.5)).unsqueeze(2)
            ...
            attention_projected = F.linear(attention, layer.o_weight)
            hidden = residual + attention_projected
            after_attention = hidden

            residual = hidden
            normalized = self.rms_norm(hidden, layer.post_norm)
            gate = F.silu(F.linear(normalized, layer.gate_weight))
            up = F.linear(normalized, layer.up_weight)
            mlp_output = F.linear(gate * up, layer.down_weight)
            hidden = residual + mlp_output
            layer_hidden.append(hidden)
            if layer_index == NUM_LAYERS - 1:
                last_scaled_scores = scaled_scores
                last_probabilities = probabilities
                last_attention_projected = attention_projected
                last_after_attention = after_attention
        ...
        return (
            ...
            position + 1,
            torch.stack(layer_hidden),
            last_scaled_scores,
            last_probabilities,
            last_attention_projected,
            last_after_attention,
            last_mlp_output,
        )
...
def bf16_bit_metrics(actual: np.ndarray, expected: np.ndarray) -> dict:
    return tensor_metrics(
        (actual.astype(np.uint32) << 16).view(np.float32),
        (expected.astype(np.uint32) << 16).view(np.float32),
    )
...
def register_custom_ops(exact_qk_source: Path, barrier_source: Path) -> None:
...
def official_eager_reference(model_dir: Path, token_ids: list[int]) -> list[dict]:
    ...
    cache = None
    steps = []
    captured_layers = [None] * NUM_LAYERS

    def make_hook(index: int):
        def hook(_module, _inputs, output):
            hidden = output[0] if isinstance(output, tuple) else output
            captured_layers[index] = bf16_bits(hidden)
    ...
    def attention_hook(_module, _inputs, output):
        captured_components["attention_projected_bits"] = bf16_bits(output[0])
        captured_components["probabilities_bits"] = bf16_bits(output[1])

    def after_attention_hook(_module, inputs):
        captured_components["after_attention_bits"] = bf16_bits(inputs[0])

    def mlp_hook(_module, _inputs, output):
        captured_components["mlp_output_bits"] = bf16_bits(output)

    component_hooks = [
        last_layer.self_attn.register_forward_hook(attention_hook),
        last_layer.post_attention_layernorm.register_forward_pre_hook(after_attention_hook),
        last_layer.mlp.register_forward_hook(mlp_hook),
    ]
    with torch.no_grad():
        for step, token_id in enumerate(token_ids):
            ...
            if any(value is None for value in captured_layers):
                raise RuntimeError("not all Hugging Face decoder layer hooks fired")
            if set(captured_components) != {
                "probabilities_bits",
                "attention_projected_bits",
                "after_attention_bits",
                "mlp_output_bits",
            }:
                raise RuntimeError("not all Hugging Face layer-27 component hooks fired")
            steps.append(
                {
                    "logits": logits,
                    ...
                    "layer_hidden_bits": np.stack(captured_layers),
                    **captured_components,
                }
            )
    for hook in hooks:
...
def eager_screen(
    ...
    register_custom_ops(exact_qk_source, barrier_source)
    ...
    with torch.no_grad():
        for step, token_id in enumerate(token_ids, start=1):
            ...
            layer_hidden_bits = bf16_bits(layer_hidden)
            manual_probability_bits = bf16_bits(last_probabilities)[..., :step]
            active_probability_bits = bf16_bits(
                torch.softmax(last_scaled_scores[..., :step].float(), dim=-1).to(torch.bfloat16)
            )
            component_bits = {
                "probabilities": manual_probability_bits,
                "attention_projected": bf16_bits(last_attention_projected),
                "after_attention": bf16_bits(last_after_attention),
                "mlp_output": bf16_bits(last_mlp_output),
            }
            ...
            layer_metrics = [
                bf16_bit_metrics(
                    layer_hidden_bits[index], official[step - 1]["layer_hidden_bits"][index]
                )
                for index in range(NUM_LAYERS)
            ]
            component_metrics = {
                name: bf16_bit_metrics(
                    bits, official[step - 1][f"{name}_bits"]
                )
                for name, bits in component_bits.items()
            }
            component_metrics["active_probabilities"] = bf16_bit_metrics(
                active_probability_bits, official[step - 1]["probabilities_bits"]
            )
            ...
            item = {
                ...
                "first_layer_hidden_mismatch": next(
                    (index for index, metric in enumerate(layer_metrics) if not metric["all_exact"]),
                    None,
                ),
                "first_layer_hidden_tolerance_failure": next(
                    (
                        index
                        for index, metric in enumerate(layer_metrics)
                        if not metric["all_within_tolerance"]
                    ),
                    None,
                ),
                "layer_hidden_vs_hf": layer_metrics,
                "last_layer_components_vs_hf": component_metrics,
            }
            item["pass"] = (
            ...
            reference[f"step{step}_hf_probabilities_bits"] = official[step - 1][
                "probabilities_bits"
            ]
            reference[f"step{step}_manual_probabilities_bits"] = manual_probability_bits
            reference[f"step{step}_active_probabilities_bits"] = active_probability_bits
            reference[f"step{step}_manual_scaled_scores"] = (
                last_scaled_scores.detach().float().cpu().numpy().copy()
            )
            previous_key = flat_key.copy()
            previous_value = flat_value.copy()
    output_dir.mkdir(parents=True, exist_ok=True)
    reference_path = output_dir / "attempt53j-eager-reference.npz"
    np.savez(reference_path, **reference)
    result = {
        "gate": "G4a Attempt 53j softmax reduction-extent localization",
        ...
        "steps": step_results,
        "claim_boundary": "Complete B=1 decoder eager prerequisite only; AIR, Device UDF and batching are not passed.",
    }
    result_path = output_dir / "eager-screen.json"
    result_path.write_text(json.dumps(result, indent=2, ensure_ascii=True) + "\n", encoding="utf-8")
    print("G4A_ATTEMPT53_EAGER " + json.dumps(result, ensure_ascii=True), flush=True)
    if not result["pass"]:
        raise SystemExit(1)
    return result
...
def export_air(
    ...
    dynamo_export(
        ...
        export_name="qwen_full_decoder_step_attempt53d",
        dynamic=False,
        config=config,
    )
    air = output_dir / "qwen_full_decoder_step_attempt53d.air"
    graph = output_dir / "dynamo.pbtxt"
    external_files = [path for path in output_dir.iterdir() if path.is_file() and path.suffix == ""]
    result = {
        "gate": "G4a Attempt 53d barrier-protected complete decoder AIR export",
        ...
        "reference_sha256": sha256(reference_path),
        "claim_boundary": "AIR export only; native GE correctness must still pass before G4a.",
    }
    (output_dir / "export-result.json").write_text(
        json.dumps(result, indent=2, ensure_ascii=True) + "\n", encoding="utf-8"
    )
    print("G4A_ATTEMPT53_EXPORT " + json.dumps(result, ensure_ascii=True), flush=True)
...
def main() -> None:
    ...
    if args.mode == "audit":
        return
    if args.exact_qk_source is None or args.barrier_source is None:
        parser.error("--exact-qk-source and --barrier-source are required for eager/export")
    torch.npu.set_device(0)
    if args.mode == "eager":
        eager_screen(args.model_dir, args.exact_qk_source, args.barrier_source, args.output_dir)
    else:

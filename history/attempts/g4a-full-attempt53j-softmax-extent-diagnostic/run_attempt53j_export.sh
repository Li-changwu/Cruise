...
printf 'Attempt 53j is an eager-only diagnostic and must not be exported.\n' >&2
exit 98

root=/root/ascend-control-g4-20260723
g2g=/root/ascend-control-g2g-20260719
src=${root}/attempt53d-src
eager=${root}/raw-attempt53d-eager
export_dir=${root}/attempt53d-export
...
if [[ -e "${export_dir}" ]]; then exit 97; fi
...
timeout 10800 python "${src}/full_decoder_step.py" --mode export --model-dir "${model}" \
  ...
  --reference "${eager}/attempt53d-eager-reference.npz" \
...
sha256sum "${src}"/* "${eager}/attempt53d-eager-reference.npz" \
  "${export_dir}/qwen_full_decoder_step_attempt53d.air" "${export_dir}/dynamo.pbtxt" \

...
src=${root}/attempt67c-src
raw=${root}/raw-attempt67c-b2-native
build=${root}/build-attempt67c-b2-native
...
sha256sum "${src}"/*.py "${src}"/*.cpp "${src}"/*.md "${src}"/*.sh \
  ...
  "${validation}/graph-inspection.json" >"${raw}/input-integrity.log"
...
python "${src}/compare_b2_native.py" --native-dir "${outputs}" \
  --reference "${reference}" --output-npz "${raw}/attempt67c-native-output.npz" \
  --output "${raw}/attempt67c-result.json" >"${raw}/compare.stdout.log" 2>&1
compare_status=$?
...
sha256sum "${air}" "${raw}/attempt67c-native-output.npz" \
  "${raw}/attempt67c-result.json" "${raw}/linear-launches.json" \
  >"${raw}/result-integrity.log" 2>/dev/null || true
cat "${raw}/attempt67c-result.json" 2>/dev/null || true

# G4a Attempt 53k: Mask-Correct Fixed-B=1 Complete Qwen Decoder Step
...
Attempt 53j showed that the previous `-10000` padding sentinel was greater than
valid layer-27 attention scores of `-11392` and `-11328`, so invalid cache slots
captured the softmax probability mass. This attempt changes only that sentinel
to the minimum finite BF16 value.
...
- Invalid logical slots use `torch.finfo(torch.bfloat16).min`; no valid score
  can be ordered below the padding sentinel.

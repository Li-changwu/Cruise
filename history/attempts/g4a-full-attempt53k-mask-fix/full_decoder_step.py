...
def eager_screen(
    ...
    reference_path = output_dir / "attempt53k-eager-reference.npz"
    np.savez(reference_path, **reference)
    result = {
        "gate": "G4a Attempt 53k mask-correct complete-decoder eager screen",
...
def export_air(
    ...
    dynamo_export(
        ...
        export_name="qwen_full_decoder_step_attempt53k",
        dynamic=False,
        config=config,
    )
    air = output_dir / "qwen_full_decoder_step_attempt53k.air"
    graph = output_dir / "dynamo.pbtxt"
    external_files = [path for path in output_dir.iterdir() if path.is_file() and path.suffix == ""]
    result = {
        "gate": "G4a Attempt 53k mask-correct complete decoder AIR export",

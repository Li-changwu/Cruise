...
src=${root}/attempt66a-r2-src
raw=${root}/raw-attempt66a-r2-dataflow-smoke
cache=${root}/cache-attempt66a-r2-dataflow-smoke
...
timeout 1800 python "${src}/dataflow_full_decoder_smoke.py" \
  ...
  --output "${raw}/attempt66a-r2-result.json" >"${raw}/smoke.stdout.log" 2>&1
smoke_status=$?
...
sha256sum "${raw}/attempt66a-r2-result.json" >"${raw}/result-integrity.log" 2>/dev/null || true
cat "${raw}/attempt66a-r2-result.json" 2>/dev/null || tail -240 "${raw}/smoke.stdout.log"

...
MAX_STEPS = 8
BLOCK_SIZE = 128
CACHE_SHAPE = (28, 8, 128, 4, 128)
FLAT_CACHE_SHAPE = (28, 1024, 4, 128)
...
CASE_SPECS = (
    ...
    ("k4-heterogeneous", 4),
    ("k8-all-active", 8),
    ("active-empty-alternating", 4),
    ("finished-active-empty-active", 4),
...
def load_initial(path: Path) -> dict:
    return {
        "token": np.fromfile(path / "token_id.bin", dtype=np.int64).reshape(BATCH_SIZE),
        "position": np.fromfile(path / "position.bin", dtype=np.int64).reshape(BATCH_SIZE),
        "length": np.fromfile(path / "sequence_length.bin", dtype=np.int32).reshape(BATCH_SIZE),
        "key": np.fromfile(path / "key_cache.bin", dtype=np.uint16).reshape(CACHE_SHAPE),
        "slot": np.fromfile(path / "slot_mapping.bin", dtype=np.int32).reshape(BATCH_SIZE),
        "active": np.fromfile(path / "active_mask.bin", dtype=np.int32).reshape(BATCH_SIZE),
        "blocks": np.fromfile(path / "block_table.bin", dtype=np.int32).reshape(BATCH_SIZE, 2),
...
def load_output(path: Path) -> dict:
    return {
        ...
        "key": np.fromfile(path / "key_cache.bin", dtype=np.uint16).reshape(CACHE_SHAPE),
        "value": np.fromfile(path / "value_cache.bin", dtype=np.uint16).reshape(CACHE_SHAPE),
        "token": np.fromfile(path / "final_token.bin", dtype=np.int64).reshape(BATCH_SIZE),
        "position": np.fromfile(path / "final_position.bin", dtype=np.int64).reshape(BATCH_SIZE),
        "length": np.fromfile(path / "final_sequence_length.bin", dtype=np.int32).reshape(BATCH_SIZE),
        "slot": np.fromfile(path / "final_slot_mapping.bin", dtype=np.int32).reshape(BATCH_SIZE),
        "active": np.fromfile(path / "final_active_mask.bin", dtype=np.int32).reshape(BATCH_SIZE),
        "control": np.fromfile(path / "control.bin", dtype=np.int32).reshape(24),
...
def expected_state(initial: dict, host: dict, max_steps: int, eos: np.ndarray) -> dict:
    ...
    reason = np.zeros(BATCH_SIZE, dtype=np.int32)
    ...
    control = np.concatenate(
        (
            np.asarray([max_steps, 0, 0, 0, model_calls, 0], dtype=np.int32),
            eos.astype(np.int32),
            initial["active"].astype(np.int32),
            executed.astype(np.int32),
            reason.astype(np.int32),
            np.asarray(
                [
...
def eager_continuity(host: dict, device: dict, reference_path: Path) -> dict:
    with np.load(reference_path) as reference:
        # Attempt 69b retained `_b2_` in its archive keys, but these are the
        # frozen tensors from the accepted true B=4 eager run.
        expected_logits = reference["case0_b2_logits"].reshape(
    ...
    result = {
        "host_logits_vs_b4_eager": metrics(host["logits"][0], expected_logits),
        "device_logits_vs_b4_eager": metrics(device["logits"][0], expected_logits),
        "host_key_vs_b4_eager": bf16_metrics(host["key"], expected_key),
        "device_key_vs_b4_eager": bf16_metrics(device["key"], expected_key),
        "host_value_vs_b4_eager": bf16_metrics(host["value"], expected_value),
        "device_value_vs_b4_eager": bf16_metrics(device["value"], expected_value),
    ...
    result["pass"] = bool(
        all(
            ...
            for key in (
                "host_logits_vs_b4_eager",
                "device_logits_vs_b4_eager",
                "host_key_vs_b4_eager",
                "device_key_vs_b4_eager",
                "host_value_vs_b4_eager",
                "device_value_vs_b4_eager",
            )
        )
...
def main() -> None:
    ...
    early_values = np.asarray(
        [
            int(value)
            for value in (args.host_dir / "early_eos_tokens.txt")
            .read_text(encoding="utf-8")
            .split()
        ],
        dtype=np.int32,
    )
    if early_values.shape != (BATCH_SIZE,):
        raise RuntimeError("expected four early EOS values")
    cases = []
    loaded = {}
    for name, max_steps in CASE_SPECS:
        ...
        eos = (
            early_values
            if name == "independent-early-eos"
            else np.full(BATCH_SIZE, CONFIGURED_EOS, dtype=np.int32)
        )
    ...
    independent_eos_pass = bool(
        ...
        and early["executed_steps"][2] == 1
        and early["finish_reason"][2] == 1
        and early["executed_steps"][1] == 4
        and early["finish_reason"][1] == 2
        and early["executed_steps"][3] == 4
        and early["finish_reason"][3] == 2
    )
    result = {
        "gate": "G4c Attempt 69e B=4 Device UDF resident generation epoch",
        ...
        "cases": cases,
        "claim_boundary": (
            "This closes the B=4 correctness and one-Feed/one-Fetch resident "
            "epoch sub-gate only. Stable performance benefit, recovery, and "

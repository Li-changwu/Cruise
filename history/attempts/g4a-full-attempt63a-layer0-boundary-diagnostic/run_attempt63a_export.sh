...
src=${root}/attempt63a-src
eager=${root}/raw-attempt63a-eager
raw=${root}/raw-attempt63a-export
export_dir=${root}/export-attempt63a
cache=${root}/cache-attempt63a-export
...
sha256sum "${src}"/*.py "${src}"/*.md "${src}"/*.sh \
  "${eager}/attempt63a-eager-reference.npz" "${baseline}" \
...
timeout 10800 python "${src}/full_decoder_step.py" --mode export --model-dir "${model}" \
  ...
  --reference "${eager}/attempt63a-eager-reference.npz" \
...
python "${src}/compare_eager_subset.py" --baseline "${baseline}" \
  --candidate "${eager}/attempt63a-eager-reference.npz" \
...
sha256sum "${export_dir}/qwen_full_decoder_step_attempt63a.air" \

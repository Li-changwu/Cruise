...
class PagedQwenDecoderStep(torch.nn.Module):
    ...
    def forward(
        ...
        for layer_index, layer in enumerate(self.layers):
            residual = hidden
            input_norm = self.rms_norm(hidden, layer.input_norm)
            normalized_2d = input_norm.reshape(-1, HIDDEN_SIZE)
            ...
            attention = attention_value.transpose(1, 2).reshape(1, 1, HIDDEN_SIZE)
            ...
            hidden = hidden_after_attention

            residual = hidden
            post_attention_norm = self.rms_norm(hidden, layer.post_norm)
            normalized_2d = post_attention_norm.reshape(-1, HIDDEN_SIZE)
            ...
            hidden_after_mlp = residual + mlp_projection
...
def eager_screen(
    ...
    with torch.no_grad():
        for step, token_id in enumerate(token_ids, start=1):
            ...
            logits, key, value, next_position, layer_hiddens = outputs[:5]
            layer0_boundaries = outputs[5:]
            ...
            for name, boundary in zip(LAYER0_BOUNDARY_NAMES, layer0_boundaries):
                reference[f"step{step}_layer0_{name}_bits"] = bf16_bits(boundary)
            previous_key = flat_key.copy()
            previous_value = flat_value.copy()
    output_dir.mkdir(parents=True, exist_ok=True)
    reference_path = output_dir / "attempt63a-eager-reference.npz"
    np.savez(reference_path, **reference)
    result = {
        "gate": "G4a Attempt 63a layer-0 boundary diagnostic eager screen",
    ...
    print("G4A_ATTEMPT63A_EAGER " + json.dumps(result, ensure_ascii=True), flush=True)
    if not result["pass"]:
        raise SystemExit(1)
    return result
...
def export_air(
    ...
    dynamo_export(
        ...
        export_name="qwen_full_decoder_step_attempt63a",
        dynamic=False,
        config=config,
    )
    air = output_dir / "qwen_full_decoder_step_attempt63a.air"
    graph = output_dir / "dynamo.pbtxt"
    external_files = [path for path in output_dir.iterdir() if path.is_file() and path.suffix == ""]
    result = {
        "gate": "G4a Attempt 63a layer-0 boundary diagnostic AIR export",
    ...
    print("G4A_ATTEMPT63A_EXPORT " + json.dumps(result, ensure_ascii=True), flush=True)

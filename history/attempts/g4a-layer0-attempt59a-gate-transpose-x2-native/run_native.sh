...
src=${root}/attempt59a-native-src
export_dir=${root}/export-attempt59a
raw=${root}/raw-attempt59a-native
cache=${root}/cache-attempt59a-native
inputs=${raw}/native-inputs
outputs=${raw}/native-outputs
air=${export_dir}/qwen_layer0_boundary_attempt59a.air
reference=${export_dir}/attempt59a-eager-reference.npz
baseline_reference=${root}/export-attempt58a/attempt58a-eager-reference.npz
baseline_native=${root}/raw-attempt58a-native/attempt58a-native-output.npz
...
g++ -D_GLIBCXX_USE_CXX11_ABI=0 -O2 -std=c++11 -ftrapv -fstack-protector-all \
  ...
  "${src}/native_layer0_attempt59a_host.cpp" -Wl,--whole-archive \
  ...
  -o "${raw}/native_layer0_attempt59a_host" >"${raw}/compile.log" 2>&1
s=$?; printf 'compile\t%s\n' "$s" >>"${status}"; [[ $s -eq 0 ]] || exit $s

timeout 1200 "${raw}/native_layer0_attempt59a_host" "${air}" "${inputs}" "${outputs}" \
...
python "${src}/compare_native.py" --native-dir "${outputs}" --reference "${reference}" \
  ...
  --output-npz "${raw}/attempt59a-native-output.npz" \
  --output "${raw}/attempt59a-result.json" >"${raw}/compare.stdout.log" 2>&1
compare_status=$?
printf 'compare\t%s\n' "${compare_status}" >>"${status}"

python "${src}/compare_boundary.py" --baseline-native "${baseline_native}" \
  --baseline-reference "${baseline_reference}" \
  --candidate-native "${raw}/attempt59a-native-output.npz" \
  --candidate-reference "${reference}" \
  --output "${raw}/attempt59a-mechanism.json" >"${raw}/mechanism.stdout.log" 2>&1
localization_status=$?
printf 'gate-transpose-x2\t%s\n' "${localization_status}" >>"${status}"
...
python "${src}/extract_gate_launch.py" --log "${raw}/native.stdout.log" \
  --output "${raw}/gate-launch.json" >"${raw}/gate-launch.stdout.log" 2>&1
gate_launch_status=$?
printf 'gate-launch\t%s\n' "${gate_launch_status}" >>"${status}"
...
sha256sum "${air}" "${raw}/attempt59a-native-output.npz" \
  "${raw}/attempt59a-result.json" "${raw}/attempt59a-mechanism.json" \
  "${raw}/gate-launch.json" \
...
if [[ ${gate_launch_status} -ne 0 ]]; then exit ${gate_launch_status}; fi

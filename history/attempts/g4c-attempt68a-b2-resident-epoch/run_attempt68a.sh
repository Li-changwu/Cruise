...
src=${root}/attempt68a-src
raw=${root}/raw-attempt68a-b2-resident-epoch
cache=${root}/cache-attempt68a-b2-resident-epoch
build=${root}/build-attempt68a-b2-resident-epoch
weights=${root}/external-weights-attempt68a
air=${root}/export-attempt67b-b2/qwen_b2_decoder_step_attempt67b.air
reference=${root}/raw-attempt67a-b2-eager/outputs/attempt67a-b2-eager-reference.npz
acceptance=${root}/raw-attempt67e-r1-b2-native-acceptance/attempt67e-r1-acceptance.json
inputs=${raw}/inputs
host_outputs=${raw}/host
device_outputs=${raw}/device
cases=(k1-heterogeneous k2-heterogeneous k4-heterogeneous k8-both-active \
       active-empty finished-active independent-early-eos)
...
timeout 10800 "${build}/g4c_b2_epoch_runner" host "${air}" \
  "${src}/config/graph_config.json" \
  "${src}/config/g4c_b2_resident_epoch_func.json" \
  "${inputs}" "${host_outputs}" -1 -1 >"${raw}/host.stdout.log" 2>&1
host_status=$?
...
if [[ ${#early_eos[@]} -ne 2 ]] ||
   ! [[ ${early_eos[0]} =~ ^[0-9]+$ && ${early_eos[1]} =~ ^[0-9]+$ ]]; then
...
timeout 10800 "${build}/g4c_b2_epoch_runner" device "${air}" \
  "${src}/config/graph_config.json" \
  "${src}/config/g4c_b2_resident_epoch_func.json" \
  "${inputs}" "${device_outputs}" "${early_eos[0]}" "${early_eos[1]}" \
...
python3 "${src}/compare_epochs.py" --host-dir "${host_outputs}" \
  ...
  --eager-reference "${reference}" --output "${raw}/attempt68a-result.json" \
...
sha256sum "${raw}/attempt68a-result.json" "${raw}/status.tsv" \
  >"${raw}/result-integrity.log" 2>/dev/null || true
cat "${raw}/attempt68a-result.json" 2>/dev/null || \

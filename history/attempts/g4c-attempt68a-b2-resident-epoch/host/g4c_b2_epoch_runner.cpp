...
const std::array<InputSpec, 9> kInputSpecs = {{
    {"token_id.bin", {2, 1}, ge::DT_INT64,
     kBatchSize * sizeof(int64_t)},
    {"position.bin", {2}, ge::DT_INT64, kBatchSize * sizeof(int64_t)},
    {"sequence_length.bin", {2, 1}, ge::DT_INT32,
     kBatchSize * sizeof(int32_t)},
    {"key_cache.bin", {28, 4, 128, 4, 128}, ge::DT_BF16, kCacheBytes},
    {"slot_mapping.bin", {2}, ge::DT_INT32,
     kBatchSize * sizeof(int32_t)},
    {"active_mask.bin", {2}, ge::DT_INT32,
     kBatchSize * sizeof(int32_t)},
    {"block_table.bin", {2, 2}, ge::DT_INT32,
     kBatchSize * kBlocksPerRequest * sizeof(int32_t)},
    {"value_cache.bin", {28, 4, 128, 4, 128}, ge::DT_BF16,
     kCacheBytes},
...
const std::array<const char *, 6> kRegularCases = {{
    ...
    "k8-both-active", "active-empty", "finished-active"}};
...
bool SaveHostCase(const std::string &output_dir,
                  ...
                  const std::array<int32_t, 16> &control,
...
int RunHostCase(const std::shared_ptr<ge::Session> &session,
  ...
  const std::array<int32_t, 16> control = {{
      ...
      reason[0],
      reason[1],
      initial_active[0] + initial_active[1],
      CountActive(active),
  }};
  ...
  if (!SaveHostCase(output_dir, logits_history, token_history, state, control,
                    model_calls, wall_us, cpu_us)) {
    return 25;
  }
  std::cout << "HOST_B2_CASE name=" << output_dir
            << " calls=" << model_calls << " executed=" << executed[0]
            << ',' << executed[1] << " reason=" << reason[0] << ','
            << reason[1] << " wall_us=" << wall_us << std::endl;
  return 0;
}

int RunHostSuite(const std::string &air_path, const std::string &input_root,
                 const std::string &output_root) {
  ge::Graph graph("Attempt68aB2HostEpoch");
  ...
  const std::array<int32_t, kBatchSize> configured =
      {{kConfiguredEos, kConfiguredEos}};
  std::array<int64_t, kBatchSize> first_tokens = {{-1, -1}};
  ...
  if (first_tokens[0] < 0 || first_tokens[0] >= kVocabSize ||
      first_tokens[1] < 0 || first_tokens[1] >= kVocabSize) {
    session.reset();
    ge::GEFinalize();
    return 14;
  }
  ...
  const std::array<int32_t, kBatchSize> early_eos =
      {{static_cast<int32_t>(first_tokens[0]), kConfiguredEos}};
  ...
  std::cout << "HOST_B2_SUITE_COMPLETE early_eos=" << first_tokens[0]
            << ',' << kConfiguredEos << std::endl;
  return 0;
}
...
ge::dflow::FlowGraph BuildDeviceFlow(const std::string &air_path,
  ...
  auto graph_pp = ge::dflow::GraphPp(
      "attempt68a_b2_decoder_graph_pp", [air_path]() {
        ge::Graph graph("Attempt68aB2InvokedDecoder");
  ...
  auto function_pp = ge::dflow::FunctionPp("g4c_b2_resident_epoch_pp")
                         .SetCompileConfig(func_config.c_str());
  function_pp.AddInvokedClosure("decode_graph_0", graph_pp);
  auto node = ge::dflow::FlowNode("g4c_b2_resident_epoch_node", 10, 10);
  node.AddPp(function_pp)
      ...
      .SetInput(8, data8)
      .SetInput(9, control);
  ge::dflow::FlowGraph flow_graph("attempt68a_b2_resident_epoch");
...
int RunDeviceCase(const std::shared_ptr<ge::Session> &session,
                  ...
                  const std::array<int32_t, kBatchSize> &eos) {
  auto buffers = base;
  std::array<int32_t, 5> control_values =
      {{max_steps, eos[0], eos[1], 0, 0}};
  ...
  inputs.push_back(MakeTensor(control_bytes, {5}, ge::DT_INT32));
  ...
  const int64_t feed_us =
      std::chrono::duration_cast<std::chrono::microseconds>(feed_end - wall_start)
          .count();
  std::cout << "DEVICE_B2_CASE name=" << output_dir
            << " max_steps=" << max_steps << " eos=" << eos[0] << ','
            << eos[1] << " feed_us=" << feed_us << " total_us=" << wall_us
...
int RunDeviceSuite(const std::string &air_path,
  ...
  std::map<ge::AscendString, ge::AscendString> options = {
      ...
      {"ge.externalWeightDir",
       "/root/ascend-control-g4-20260723/external-weights-attempt68a"},
      {"ge.graphRunMode", "0"}};
  auto ret = ge::GEInitialize(options);
  ...
  ge::GEFinalize();
  if (early_status != 0) return early_status;
  std::cout << "DEVICE_B2_SUITE_COMPLETE early_eos=" << early_eos[0] << ','
            << early_eos[1] << std::endl;
  return 0;
}
}  // namespace
...
int main(int argc, char **argv) {
  if (argc != 9) {
    std::cerr << "usage: g4c_b2_epoch_runner ROUTE AIR GRAPH_CONFIG "
  ...
  if (route == "device") {
    const std::array<int32_t, kBatchSize> early_eos = {{
        static_cast<int32_t>(std::stol(argv[7])),
        static_cast<int32_t>(std::stol(argv[8])),
    }};
    if (early_eos[0] < 0 || early_eos[0] >= kVocabSize ||
        early_eos[1] < 0 || early_eos[1] >= kVocabSize) {
      return 3;
    }

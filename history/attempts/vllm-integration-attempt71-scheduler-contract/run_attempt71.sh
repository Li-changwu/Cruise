...
src=${root}/attempt71-src
evidence=${root}/evidence-attempt71r9-scheduler-native-sidecar
scratch=/dev/shm/a71r9
...
runtime_air=${scratch}/qwen_b4_decoder_step_attempt71r9.air
cache=${scratch}/cache
...
run_step relocate-runtime-air 600s "${build}/relocate_air_paths" \
  ...
  "${runtime_export}" "${evidence}/attempt71r9-air-relocation.json"
relocate_status=$?
...
run_step scheduler-native 10800s python3 "${src}/run_scheduler_native.py" \
  --model-config "${model_config}" \
  --baseline-result "${baseline}" \
  --output "${evidence}/attempt71r9-native-result.json"
native_status=$?
...
idle_status=$?
capture_after
sha256sum "${evidence}/attempt71r9-native-result.json" "${status}" \
  "${evidence}/native-bridge-integrity.log" \
...
finalized=1
cat "${evidence}/attempt71r9-native-result.json"
trap - EXIT

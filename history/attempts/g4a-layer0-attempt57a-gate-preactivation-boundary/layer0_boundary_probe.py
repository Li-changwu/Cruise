...
class Layer0BoundaryProbe(torch.nn.Module):
    ...
    def forward(
        ...
        gate_preactivation = torch.ops.g4a_materialize.materialize(
            F.linear(post_attention_norm, self.gate_weight)
        )
...
def main() -> None:
    ...
    reference_path = args.output_dir / "attempt57a-eager-reference.npz"
    ...
    dynamo_export(
        ...
        export_name="qwen_layer0_boundary_attempt57a",
        dynamic=False,
        config=config,
    )
    air = args.output_dir / "qwen_layer0_boundary_attempt57a.air"

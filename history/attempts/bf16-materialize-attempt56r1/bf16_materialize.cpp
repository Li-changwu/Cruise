    ...
    GM_ADDR gm_x, GM_ADDR gm_y, GM_ADDR workspace, GM_ADDR gm_tiling_data)
{
    ...
    const uint32_t aligned_elements = elements / kElementsPerBlock * kElementsPerBlock;
    if (aligned_elements != 0) {
        ...
        event_t mte2_to_mte3 = static_cast<event_t>(

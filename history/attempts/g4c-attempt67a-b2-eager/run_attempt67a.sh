...
src=${root}/attempt67a-src
raw=${root}/raw-attempt67a-b2-eager
...
timeout 7200 python "${src}/b2_eager_test.py" \
  --model-dir "${model}" \
...
sha256sum "${raw}/outputs/attempt67a-b2-eager-reference.npz" \
  "${raw}/outputs/attempt67a-b2-eager-result.json" \
  "${raw}/outputs/checkpoint-audit.json" >"${raw}/result-integrity.log"
cat "${raw}/outputs/attempt67a-b2-eager-result.json"
exit ${idle_status}

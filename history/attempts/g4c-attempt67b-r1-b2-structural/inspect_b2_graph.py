...
def main() -> None:
    ...
    for name, op, block in nodes:
        if op != "Data" or 'dtype: DT_INT32' not in block:
            continue
        if descriptor_shape(block, "[o]y") != (2,):
            continue
        consumers = []
        for consumer_name, consumer_op, consumer_block in nodes:
            inputs = re.findall(
                r'^\s+input: "([^"]*)"', consumer_block, re.MULTILINE
            )
            if any(value.split(":", 1)[0] == name for value in inputs):
                consumers.append({"name": consumer_name, "op": consumer_op})
        int32_vector_data.append(
            {"name": name, "direct_consumer_count": len(consumers), "consumers": consumers}
        )
    active_data = [
        item for item in int32_vector_data if item["direct_consumer_count"] == 113
    ]
    slot_data = [
        item for item in int32_vector_data if item["direct_consumer_count"] == 112
    ]
    result = {
        "valid": (
        ...
        "active_mask_data_count": len(active_data),
        "active_mask_data": active_data,
        "slot_mapping_data_count": len(slot_data),
        "slot_mapping_data": slot_data,

...
CANDIDATE_OUTPUT_NAMES = (
    *BASELINE_OUTPUT_NAMES[: BASELINE_OUTPUT_NAMES.index("gate")],
    "gate_preactivation",
    *BASELINE_OUTPUT_NAMES[BASELINE_OUTPUT_NAMES.index("gate") :],
)
...
def main() -> None:
    ...
    step1_preactivation_exact = steps[0]["candidate_outputs"]["gate_preactivation"][
        "all_exact"
    ]
    ...
    if not common_eager_references_exact or not native_prefix_exact:
        localization = "confounded"
    elif not step1_preactivation_exact:
        localization = "gate_matmul"
    elif not step1_gate_exact:
        localization = "silu"
    elif steps[0]["candidate_first_exact_mismatch"] is None:
        localization = "step1_exact"
    else:
        localization = "after_silu"
    ...
    diagnostic_valid = (
        ...
        and localization != "confounded"
    )
    result = {
        "gate": "Attempt 57a gate MatMul-to-SiLU localization",
        ...
        "steps": steps,
        "claim_boundary": (
            "Layer-0 gate projection versus SiLU only; this cannot pass complete G4a."
        ),
    }

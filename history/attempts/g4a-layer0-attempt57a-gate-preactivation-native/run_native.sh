...
src=${root}/attempt57a-native-src
export_dir=${root}/export-attempt57a
raw=${root}/raw-attempt57a-native
cache=${root}/cache-attempt57a-native
inputs=${raw}/native-inputs
outputs=${raw}/native-outputs
air=${export_dir}/qwen_layer0_boundary_attempt57a.air
reference=${export_dir}/attempt57a-eager-reference.npz
...
g++ -D_GLIBCXX_USE_CXX11_ABI=0 -O2 -std=c++11 -ftrapv -fstack-protector-all \
  ...
  "${src}/native_layer0_attempt57a_host.cpp" -Wl,--whole-archive \
  ...
  -o "${raw}/native_layer0_attempt57a_host" >"${raw}/compile.log" 2>&1
s=$?; printf 'compile\t%s\n' "$s" >>"${status}"; [[ $s -eq 0 ]] || exit $s

timeout 1200 "${raw}/native_layer0_attempt57a_host" "${air}" "${inputs}" "${outputs}" \
...
python "${src}/compare_native.py" --native-dir "${outputs}" --reference "${reference}" \
  ...
  --output-npz "${raw}/attempt57a-native-output.npz" \
  --output "${raw}/attempt57a-result.json" >"${raw}/compare.stdout.log" 2>&1
compare_status=$?
printf 'compare\t%s\n' "${compare_status}" >>"${status}"

python "${src}/compare_boundary.py" --baseline-native "${baseline_native}" \
  --baseline-reference "${baseline_reference}" \
  --candidate-native "${raw}/attempt57a-native-output.npz" \
  --candidate-reference "${reference}" \
  --output "${raw}/attempt57a-localization.json" >"${raw}/localization.stdout.log" 2>&1
localization_status=$?
printf 'gate-localization\t%s\n' "${localization_status}" >>"${status}"
...
sha256sum "${air}" "${raw}/attempt57a-native-output.npz" \
  "${raw}/attempt57a-result.json" "${raw}/attempt57a-localization.json" \
  >"${raw}/result-integrity.log" 2>/dev/null || true
cat "${raw}/compare.stdout.log"
cat "${raw}/localization.stdout.log"

...
UNIQUE_INPUTS = {
    ("DT_INT64", (2, 1)): "token_id",
    ("DT_INT64", (2,)): "position",
    ("DT_INT32", (2, 1)): "sequence_length",
    ("DT_INT32", (2, 2)): "block_table",
    ("DT_UINT8", (72,)): "explicit_tiling",
}
CACHE_SIGNATURE = ("DT_BF16", (28, 4, 128, 4, 128))
CONTROL_VECTOR_SIGNATURE = ("DT_INT32", (2,))
EXPECTED_SIGNATURES = {
    "token_id": ("DT_INT64", (2, 1)),
    "position": ("DT_INT64", (2,)),
    "sequence_length": ("DT_INT32", (2, 1)),
    "block_table": ("DT_INT32", (2, 2)),
    "slot_mapping": CONTROL_VECTOR_SIGNATURE,
    "key_cache": CACHE_SIGNATURE,
    "value_cache": CACHE_SIGNATURE,
    "explicit_tiling": ("DT_UINT8", (72,)),
...
def parse_inputs(path: Path) -> list[dict]:
    ...
    for item in result:
        ...
        item["direct_consumer_count"] = graph_text.count(
            f'input: "{item["node_name"]}:0"'
        )
        if signature == CACHE_SIGNATURE:
            ...
            if item["direct_consumer_count"] == 112:
                item["semantic"] = "slot_mapping"
            elif item["direct_consumer_count"] == 113:
...
def main() -> None:
    ...
    expected_semantics = set(EXPECTED_SIGNATURES)
    input_valid = (
        ...
        and all(
            item["semantic"] in EXPECTED_SIGNATURES
            and (item["dtype"], tuple(item["shape"]))
            == EXPECTED_SIGNATURES[item["semantic"]]
            for item in inputs
        )
    )
    ...
    result = {
        ...
        "semantic_rule": (
            "Unique dtype/shape signatures identify five inputs; cache order is "
            "validated by native recurrence; the 112-use INT32 vector is slot "
            "mapping and the 113-use vector is active mask because the extra "
            "consumer freezes next_position."
        ),
    }

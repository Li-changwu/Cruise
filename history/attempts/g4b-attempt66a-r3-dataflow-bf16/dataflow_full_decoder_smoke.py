...
def enable_bf16_numpy_bridge() -> None:
    """Register ml_dtypes.bfloat16 with the CANN DataFlow Python wrapper."""
    ...
    df_dt._dflow_dtype_to_np_dtype[df_dt.DT_BF16] = ml_dtypes.bfloat16
    df_dt._np_dtype_to_dflow_dtype[bf16_dtype] = df_dt.DT_BF16
    if ml_dtypes.bfloat16 not in df_impl._support_np_dtype:
        df_impl._support_np_dtype.append(ml_dtypes.bfloat16)
    dwrapper.init_datatype_manager(
        {
            ...
            data_wrapper.DataType.DT_BF16: np.array([], ml_dtypes.bfloat16),
...
def as_bfloat16(bits: np.ndarray) -> np.ndarray:
    return np.ascontiguousarray(bits.astype(np.uint16, copy=False)).view(
        ml_dtypes.bfloat16
    )
...
def prepare_feed_tensors(inputs: list[np.ndarray]) -> tuple[list[df.Tensor], list[str]]:
    tensors = [df.Tensor(value) for value in inputs]
    wrapper_dtypes = [str(tensor._impl.get_dtype()) for tensor in tensors]
    for index in (3, 6):
        if tensors[index]._impl.get_dtype() != data_wrapper.DataType.DT_BF16:
            raise RuntimeError(
                f"input {index} wrapper dtype is {wrapper_dtypes[index]}, not DT_BF16"
            )
    print(f"BF16_WRAPPER_PREFLIGHT {wrapper_dtypes}", flush=True)
    return tensors, wrapper_dtypes
...
def main() -> None:
    ...
    feed_tensors, wrapper_dtypes = prepare_feed_tensors(inputs)
    df.init(options)
    try:
        ...
        graph.feed_data(dict(zip(flow_inputs, feed_tensors, strict=True)))
    ...
    result = {
        "gate": "G4b Attempt 66a-r3 DataFlow full-decoder BF16 smoke",
        "pass": False,
        "air_sha256": sha256(args.air),
        "reference_sha256": sha256(args.reference),
        "input_dtypes": [str(value.dtype) for value in inputs],
        "wrapper_input_dtypes": wrapper_dtypes,
        ...
        "bf16_bridge": (
            "project-local ml_dtypes.bfloat16 registered as DT_BF16; input cache "
            "bits are reinterpreted without numeric conversion"
        ),
        "claim_boundary": (

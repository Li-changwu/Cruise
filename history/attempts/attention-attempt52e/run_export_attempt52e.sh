...
timeout 1800 python "${src}/export_attention_attempt52_air.py" \
  ...
  --custom-source "${g2g}/attempt50-src" \
  --barrier-source "${g2g}/attempt54b-probe-src" --model-dir "${model}" \
...
grep -n -A12 -B4 'op: "Bf16Barrier"' "${out}/dynamo.pbtxt" \
  >"${raw}/barrier-node.txt"
s=$?; printf 'barrier-node\t%s\n' "$s" >>"${status}"
if [[ $s -ne 0 ]]; then exit 93; fi
barrier_count=$(grep -c 'op: "Bf16Barrier"' "${out}/dynamo.pbtxt")
printf '%s\n' "${barrier_count}" >"${raw}/barrier-node-count.txt"
if [[ ${barrier_count} -ne 2 ]]; then
  printf 'barrier-node-count\t92\n' >>"${status}"; exit 92
fi
printf 'barrier-node-count\t0\n' >>"${status}"

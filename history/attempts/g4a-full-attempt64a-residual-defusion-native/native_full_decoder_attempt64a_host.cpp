...
int main(int argc, char **argv) {
  if (argc != 4) {
    std::cerr << "usage: native_full_decoder_attempt64a_host AIR INPUT_DIR OUTPUT_DIR\n";
    return 2;
  }

  ge::Graph graph("G4aAttempt64aResidualDefusionDiagnostic");
  const auto load_status = graph.LoadFromFile(argv[1]);
  std::cout << "G4A_ATTEMPT64A_LOAD status=" << load_status
  ...
  for (int step = 1; step <= 4; ++step) {
    ...
    if (ret != ge::SUCCESS || outputs.size() != 5) {
    ...
    std::cout << "G4A_ATTEMPT64A_STEP step=" << step << " outputs=" << outputs.size()
              << std::endl;
  }

  ge::GEFinalize();
  std::cout << "G4A_ATTEMPT64A_NATIVE_COMPLETE" << std::endl;
  return 0;
}

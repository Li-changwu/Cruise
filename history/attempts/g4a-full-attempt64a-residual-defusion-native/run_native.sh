...
src=${root}/attempt64a-native-src
export_dir=${root}/export-attempt64a
raw=${root}/raw-attempt64a-native
inputs=${raw}/native-inputs
outputs=${raw}/native-outputs
air=${export_dir}/qwen_full_decoder_step_attempt64a.air
reference=${root}/raw-attempt64a-eager/attempt64a-eager-reference.npz
baseline_result=${root}/raw-attempt62a-native/attempt62a-result.json
...
sha256sum "${src}"/*.py "${src}"/*.cpp "${src}"/*.sh "${src}"/*.md \
  ...
  "${export_dir}/graph-inspection.json" "${baseline_result}" \
...
g++ -D_GLIBCXX_USE_CXX11_ABI=0 -O2 -std=c++11 -ftrapv -fstack-protector-all \
  ...
  "${src}/native_full_decoder_attempt64a_host.cpp" -Wl,--whole-archive \
  ...
  -o "${raw}/native_full_decoder_attempt64a_host" >"${raw}/compile.log" 2>&1
s=$?
printf 'compile\t%s\n' "$s" >>"${status}"
[[ $s -eq 0 ]] || exit $s

timeout 3600 "${raw}/native_full_decoder_attempt64a_host" "${air}" "${inputs}" \
...
python "${src}/compare_native.py" --native-dir "${outputs}" --reference "${reference}" \
  --initial-input-dir "${inputs}" --output-npz "${raw}/attempt64a-native-output.npz" \
  --output "${raw}/attempt64a-result.json" >"${raw}/compare.stdout.log" 2>&1
compare_status=$?
printf 'compare\t%s\n' "${compare_status}" >>"${status}"

python "${src}/evaluate_mechanism.py" --baseline "${baseline_result}" \
  --candidate "${raw}/attempt64a-result.json" \
  --output "${raw}/mechanism-evaluation.json" >"${raw}/mechanism.stdout.log" 2>&1
mechanism_status=$?
printf 'residual-defusion\t%s\n' "${mechanism_status}" >>"${status}"
...
if [[ ${exact_count} -eq 28 ]]; then exact_status=0; else exact_status=94; fi
...
if [[ ${barrier_count} -eq 28 ]]; then barrier_status=0; else barrier_status=93; fi
...
if [[ ${materialize_count} -eq 56 ]]; then materialize_status=0; else materialize_status=92; fi
...
python "${src}/extract_linear_launches.py" --log "${raw}/native.stdout.log" \
  --expected 197 --output "${raw}/linear-launches.json" >"${raw}/linear-launches.stdout.log" 2>&1
linear_status=$?
...
sha256sum "${air}" "${raw}/attempt64a-native-output.npz" \
  "${raw}/attempt64a-result.json" "${raw}/mechanism-evaluation.json" \
  "${raw}/linear-launches.json" \
  >"${raw}/result-integrity.log" 2>/dev/null || true
cat "${raw}/attempt64a-result.json" 2>/dev/null || true
cat "${raw}/mechanism-evaluation.json" 2>/dev/null || true
cat "${raw}/linear-launches.json" 2>/dev/null || true
if [[ ${compare_status} -ne 0 ]]; then exit ${compare_status}; fi
if [[ ${mechanism_status} -ne 0 ]]; then exit ${mechanism_status}; fi

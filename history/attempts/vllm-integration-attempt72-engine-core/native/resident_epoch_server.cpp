...
constexpr uint16_t kShutdown = 2;
...
static_assert(sizeof(Request) == 112, "resident epoch request ABI changed");
static_assert(sizeof(Response) == 304, "resident epoch response ABI changed");
  ...
  while (true) {
    ...
    if (request.magic != kRequestMagic ||
      ...
      response.transport_status = resident_epoch_execute(
          ...
          response.token_ids, response.executed, &response.model_calls,
          &response.device_status, &response.feed_calls, &response.fetch_calls,
          &response.wall_us);
    }

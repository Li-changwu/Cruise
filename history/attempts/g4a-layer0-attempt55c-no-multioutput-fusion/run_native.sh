...
src=${root}/attempt55c-native-src
export_dir=${root}/export-attempt55a
raw=${root}/raw-attempt55c-native
cache=${root}/cache-attempt55c-native
...
g++ -D_GLIBCXX_USE_CXX11_ABI=0 -O2 -std=c++11 -ftrapv -fstack-protector-all \
  ...
  "${src}/native_layer0_attempt55c_host.cpp" -Wl,--whole-archive \
  ...
  -o "${raw}/native_layer0_attempt55c_host" >"${raw}/compile.log" 2>&1
s=$?; printf 'compile\t%s\n' "$s" >>"${status}"; [[ $s -eq 0 ]] || exit $s

timeout 1200 "${raw}/native_layer0_attempt55c_host" "${air}" "${inputs}" "${outputs}" \
  "${fusion_switch}" \
...
python "${src}/compare_native.py" --native-dir "${outputs}" --reference "${reference}" \
  ...
  --output-npz "${raw}/attempt55c-native-output.npz" \
  --output "${raw}/attempt55c-result.json" >"${raw}/compare.stdout.log" 2>&1
compare_status=$?
...
grep -iE 'fusionSwitchFile|fusion switch|TbeMultiOutputFusionPass' "${raw}/native.stdout.log" \
  >"${raw}/fusion-switch-metadata.txt" || true
if grep -q 'Start buffer fusion: TbeMultiOutputFusionPass' "${raw}/native.stdout.log" ||
   grep -q 'te_fused_op_swish_mul' "${raw}/native.stdout.log"; then
  fusion_status=92
else
  fusion_status=0
fi
printf 'multioutput-fusion-disabled\t%s\n' "${fusion_status}" >>"${status}"
...
sha256sum "${air}" "${fusion_switch}" "${raw}/attempt55c-native-output.npz" \
  "${raw}/attempt55c-result.json" >"${raw}/result-integrity.log" 2>/dev/null || true

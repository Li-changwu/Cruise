...
def prepare_host_operands(
    q_bits: np.ndarray, key_bits: np.ndarray
) -> tuple[np.ndarray, np.ndarray]:
    if q_bits.dtype != np.uint16 or key_bits.dtype != np.uint16:
        raise RuntimeError("expected raw uint16 BF16 operands")
    if q_bits.shape != (1, NUM_HEADS, 1, HEAD_DIM):
        raise RuntimeError(f"unexpected Q shape {q_bits.shape}")
    if key_bits.shape != (1, NUM_KV_HEADS, MAX_KV, HEAD_DIM):
        raise RuntimeError(f"unexpected K shape {key_bits.shape}")
    ...
    if a.shape != (1, NUM_HEADS, HEAD_DIM) or b.shape != (
        NUM_HEADS,
        HEAD_DIM,
        MAX_KV,
    ):
        raise RuntimeError(f"mapped shapes are invalid A={a.shape} B={b.shape}")
    return a, b
...
enable_custom_op()
_lib = torch.library.Library("g2g", "DEF")
_lib.define("exact_qk(Tensor a, Tensor b, Tensor explicit_tiling) -> Tensor")


def _exact_qk_npu(
    a: torch.Tensor, b: torch.Tensor, explicit_tiling: torch.Tensor
) -> torch.Tensor:
    ...
    output = torch.empty(
        (1, NUM_HEADS, MAX_KV), dtype=torch.bfloat16, device=a.device
    )
...
@register_fx_node_ge_converter(torch.ops.g2g.exact_qk.default)
...
class MinimalExactQk(torch.nn.Module):
    def forward(
        self, a: torch.Tensor, b: torch.Tensor, explicit_tiling: torch.Tensor
    ) -> torch.Tensor:
        raw = torch.ops.g2g.exact_qk(a, b, explicit_tiling)
        return (raw / math.sqrt(HEAD_DIM)).unsqueeze(2).float()
...
def main() -> None:
    ...
    parser.add_argument("--name", default="exact_qk_minimal")
    ...
    if tiling_bytes.shape != (72,):
        raise RuntimeError(f"unexpected tiling byte shape {tiling_bytes.shape}")
    tiling_path = args.native_input_dir / "tiling.bin"
    tiling_bytes.tofile(tiling_path)
    operands = []
    for step in range(1, 5):
        a, b = prepare_host_operands(
    ...
    model = MinimalExactQk().eval().npu()
    sample_a = bits_to_npu(operands[0][0])
    sample_b = bits_to_npu(operands[0][1])
    sample_tiling = torch.from_numpy(tiling_bytes).npu()
    with torch.no_grad():
        eager_output = model(sample_a, sample_b, sample_tiling)
    torch.npu.synchronize()
    eager_output_np = eager_output.cpu().numpy()
    reference = eager["step1_qk_scores"].astype(np.float32)
    eager_exact = bool(np.array_equal(eager_output_np, reference))
    if not eager_exact:
        raise RuntimeError("custom-op eager wrapper does not reproduce step-1 eager QK")
    ...
    print("G2G_ATTEMPT6_EXPORT " + json.dumps(result, ensure_ascii=True), flush=True)

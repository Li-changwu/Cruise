...
src=${root}/attempt63a-native-src
export_dir=${root}/export-attempt63a
raw=${root}/raw-attempt63a-native
inputs=${raw}/native-inputs
outputs=${raw}/native-outputs
air=${export_dir}/qwen_full_decoder_step_attempt63a.air
reference=${root}/raw-attempt63a-eager/attempt63a-eager-reference.npz
baseline_native=${root}/raw-attempt62a-native/attempt62a-native-output.npz
...
sha256sum "${src}"/*.py "${src}"/*.cpp "${src}"/*.sh "${src}"/*.md \
  ...
  "${export_dir}/graph-inspection.json" "${baseline_native}" \
...
g++ -D_GLIBCXX_USE_CXX11_ABI=0 -O2 -std=c++11 -ftrapv -fstack-protector-all \
  ...
  "${src}/native_full_decoder_attempt63a_host.cpp" -Wl,--whole-archive \
  ...
  -o "${raw}/native_full_decoder_attempt63a_host" >"${raw}/compile.log" 2>&1
s=$?
printf 'compile\t%s\n' "$s" >>"${status}"
[[ $s -eq 0 ]] || exit $s

timeout 3600 "${raw}/native_full_decoder_attempt63a_host" "${air}" "${inputs}" \
...
python "${src}/compare_native.py" --native-dir "${outputs}" --reference "${reference}" \
  --initial-input-dir "${inputs}" --output-npz "${raw}/attempt63a-native-output.npz" \
  --output "${raw}/attempt63a-result.json" >"${raw}/compare.stdout.log" 2>&1
compare_status=$?
printf 'compare\t%s\n' "${compare_status}" >>"${status}"

python "${src}/compare_common_outputs.py" --baseline "${baseline_native}" \
  --candidate "${raw}/attempt63a-native-output.npz" \
  --output "${raw}/native-common-comparison.json" >"${raw}/native-common.stdout.log" 2>&1
common_status=$?
printf 'native-common-identical\t%s\n' "${common_status}" >>"${status}"
...
sha256sum "${air}" "${raw}/attempt63a-native-output.npz" \
  "${raw}/attempt63a-result.json" "${raw}/linear-launches.json" \
  "${raw}/native-common-comparison.json" \
  >"${raw}/result-integrity.log" 2>/dev/null || true
cat "${raw}/attempt63a-result.json" 2>/dev/null || true
cat "${raw}/linear-launches.json" 2>/dev/null || true
if [[ ${compare_status} -ne 0 ]]; then exit ${compare_status}; fi
if [[ ${common_status} -ne 0 ]]; then exit ${common_status}; fi

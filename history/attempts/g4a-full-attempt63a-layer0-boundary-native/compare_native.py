...
LAYER0_BOUNDARY_SPECS = (
    ("input_norm", (1, 1, 3584)),
    ("query_rope", (1, 28, 1, 128)),
    ("key_rope", (1, 4, 1, 128)),
    ("value_projection", (1, 4, 1, 128)),
    ("masked_scores", (1, 28, 1, 8)),
    ("probabilities", (1, 28, 1, 8)),
    ("attention_value", (1, 28, 1, 128)),
    ("attention_projection", (1, 1, 3584)),
    ("hidden_after_attention", (1, 1, 3584)),
    ("post_attention_norm", (1, 1, 3584)),
    ("gate_preactivation", (1, 1, 18944)),
    ("gate", (1, 1, 18944)),
    ("up", (1, 1, 18944)),
    ("mlp_product", (1, 1, 18944)),
    ("mlp_projection", (1, 1, 3584)),
    ("hidden_after_mlp", (1, 1, 3584)),
)
...
def main() -> None:
    ...
    with np.load(args.reference) as reference:
        for step in range(1, 5):
            ...
            layer0_boundary_metrics = {}
            layer0_boundary_values = {}
            for name, shape in LAYER0_BOUNDARY_SPECS:
                value = np.fromfile(
                    args.native_dir / f"step{step}_layer0_{name}.bin", dtype=np.uint16
                ).reshape(shape)
                expected = reference[f"step{step}_layer0_{name}_bits"]
                layer0_boundary_values[name] = value
                layer0_boundary_metrics[name] = bf16_metrics(value, expected)
            item = {
                "step": step,
                ...
                "first_layer0_boundary_exact_mismatch": next(
                    (
                        name
                        for name, _ in LAYER0_BOUNDARY_SPECS
                        if not layer0_boundary_metrics[name]["all_exact"]
                    ),
                    None,
                ),
                "first_layer0_boundary_tolerance_failure": next(
                    (
                        name
                        for name, _ in LAYER0_BOUNDARY_SPECS
                        if not layer0_boundary_metrics[name]["all_within_tolerance"]
                    ),
                    None,
                ),
                "layer0_boundaries_vs_eager": layer0_boundary_metrics,
            }
            ...
            for name, value in layer0_boundary_values.items():
                saved[f"step{step}_layer0_{name}_bits"] = value
            previous_key = key_bits.copy()
            previous_value = value_bits.copy()

    np.savez(args.output_npz, **saved)
    result = {
        "gate": "G4a Attempt 63a full-graph layer-0 boundary localization",
        ...
        "diagnosis": {
            ...
            "first_layer0_boundary_exact_mismatch": [
                item["first_layer0_boundary_exact_mismatch"] for item in steps
            ],
            "first_layer0_boundary_tolerance_failure": [
                item["first_layer0_boundary_tolerance_failure"] for item in steps
            ],
        },

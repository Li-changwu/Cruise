...
constexpr size_t kLayerHiddensBytes = 28ULL * 3584ULL * sizeof(uint16_t);
constexpr size_t kHiddenBytes = 3584ULL * sizeof(uint16_t);
constexpr size_t kKvProjectionBytes = 512ULL * sizeof(uint16_t);
constexpr size_t kScoresBytes = 28ULL * 8ULL * sizeof(uint16_t);
constexpr size_t kIntermediateBytes = 18944ULL * sizeof(uint16_t);
...
int main(int argc, char **argv) {
  if (argc != 4) {
    std::cerr << "usage: native_full_decoder_attempt63a_host AIR INPUT_DIR OUTPUT_DIR\n";
    return 2;
  }

  ge::Graph graph("G4aAttempt63aLayer0BoundaryDiagnostic");
  const auto load_status = graph.LoadFromFile(argv[1]);
  std::cout << "G4A_ATTEMPT63A_LOAD status=" << load_status
  ...
  for (int step = 1; step <= 4; ++step) {
    ...
    if (ret != ge::SUCCESS || outputs.size() != 21) {
    ...
    const std::vector<std::string> names = {
        "logits", "key_cache", "value_cache", "next_position", "layer_hiddens",
        "layer0_input_norm", "layer0_query_rope", "layer0_key_rope",
        "layer0_value_projection", "layer0_masked_scores", "layer0_probabilities",
        "layer0_attention_value", "layer0_attention_projection",
        "layer0_hidden_after_attention", "layer0_post_attention_norm",
        "layer0_gate_preactivation", "layer0_gate", "layer0_up",
        "layer0_mlp_product", "layer0_mlp_projection", "layer0_hidden_after_mlp"};
    const std::vector<size_t> sizes = {
        kLogitsBytes, kCacheBytes, kCacheBytes, kPositionBytes, kLayerHiddensBytes,
        kHiddenBytes, kHiddenBytes, kKvProjectionBytes, kKvProjectionBytes,
        kScoresBytes, kScoresBytes, kHiddenBytes, kHiddenBytes, kHiddenBytes,
        kHiddenBytes, kIntermediateBytes, kIntermediateBytes, kIntermediateBytes,
        kIntermediateBytes, kHiddenBytes, kHiddenBytes};
    ...
    std::cout << "G4A_ATTEMPT63A_STEP step=" << step << " outputs=" << outputs.size()
              << std::endl;
  }

  ge::GEFinalize();
  std::cout << "G4A_ATTEMPT63A_NATIVE_COMPLETE" << std::endl;
  return 0;
}

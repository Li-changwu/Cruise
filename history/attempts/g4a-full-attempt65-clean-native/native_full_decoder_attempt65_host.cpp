...
int main(int argc, char **argv) {
  if (argc != 4) {
    std::cerr << "usage: native_full_decoder_attempt65_host AIR INPUT_DIR OUTPUT_DIR\n";
    return 2;
  }

  ge::Graph graph("G4aAttempt65CleanResidualDefusedDecoder");
  const auto load_status = graph.LoadFromFile(argv[1]);
  std::cout << "G4A_ATTEMPT65_LOAD status=" << load_status
  ...
  for (int step = 1; step <= 4; ++step) {
    ...
    const std::vector<std::string> names = {
        "logits", "key_cache", "value_cache", "next_position"};
    ...
    std::cout << "G4A_ATTEMPT65_STEP step=" << step << " outputs=" << outputs.size()
              << std::endl;
  }

  ge::GEFinalize();
  std::cout << "G4A_ATTEMPT65_NATIVE_COMPLETE" << std::endl;
  return 0;
}

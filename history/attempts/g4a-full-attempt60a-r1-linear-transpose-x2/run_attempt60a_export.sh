...
src=${root}/attempt60a-r1-src
eager=${root}/raw-attempt60a-r1-eager
raw=${root}/raw-attempt60a-r1-export
export_dir=${root}/export-attempt60a-r1
cache=${root}/cache-attempt60a-r1-export
...
if [[ ! -f "${eager}/eager-screen.json" ]] ||
   ...
   ! grep -q '"valid": true' "${eager}/eager-comparison.json"; then exit 96; fi
...
[[ -n "${custom_set_env}" && -n "${barrier_set_env}" ]] || exit 94
export ASCEND_CUSTOM_OPP_PATH=${ASCEND_CUSTOM_OPP_PATH:-}
source "${custom_set_env}"
source "${barrier_set_env}"

sha256sum "${src}"/*.py "${src}"/*.md "${src}"/*.sh \
  "${eager}/attempt60a-eager-reference.npz" "${baseline}" \
...
timeout 10800 python "${src}/full_decoder_step.py" --mode export --model-dir "${model}" \
  ...
  --reference "${eager}/attempt60a-eager-reference.npz" \
...
python "${src}/compare_eager.py" --baseline "${baseline}" \
  --candidate "${eager}/attempt60a-eager-reference.npz" \
...
sha256sum "${export_dir}/qwen_full_decoder_step_attempt60a.air" \

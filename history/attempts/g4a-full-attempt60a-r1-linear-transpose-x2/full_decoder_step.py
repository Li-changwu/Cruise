...
def eager_screen(
    ...
    reference_path = output_dir / "attempt60a-eager-reference.npz"
    np.savez(reference_path, **reference)
    result = {
        "gate": "G4a Attempt 60a transpose-x2 complete-decoder eager screen",
    ...
    print("G4A_ATTEMPT60A_EAGER " + json.dumps(result, ensure_ascii=True), flush=True)
    if not result["pass"]:
        raise SystemExit(1)
    return result
...
def export_air(
    ...
    dynamo_export(
        ...
        export_name="qwen_full_decoder_step_attempt60a",
        dynamic=False,
        config=config,
    )
    air = output_dir / "qwen_full_decoder_step_attempt60a.air"
    graph = output_dir / "dynamo.pbtxt"
    external_files = [path for path in output_dir.iterdir() if path.is_file() and path.suffix == ""]
    result = {
        "gate": "G4a Attempt 60a transpose-x2 complete decoder AIR export",
    ...
    print("G4A_ATTEMPT60A_EXPORT " + json.dumps(result, ensure_ascii=True), flush=True)

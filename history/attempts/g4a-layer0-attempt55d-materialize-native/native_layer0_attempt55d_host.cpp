...
int main(int argc, char **argv) {
  if (argc != 4) {
    std::cerr << "usage: native_layer0_attempt55d_host AIR INPUT_DIR OUTPUT_DIR\n";
    return 2;
  }
  ge::Graph graph("G4aAttempt55dLayer0Materialize");
  const auto load_status = graph.LoadFromFile(argv[1]);
  std::cout << "G4A_ATTEMPT55D_LOAD status=" << load_status
  ...
  for (int step = 1; step <= 4; ++step) {
    ...
    std::cout << "G4A_ATTEMPT55D_STEP step=" << step
              << " outputs=" << outputs.size() << std::endl;
  }
  ge::GEFinalize();
  std::cout << "G4A_ATTEMPT55D_NATIVE_COMPLETE" << std::endl;
  return 0;
}

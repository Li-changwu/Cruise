...
src=${root}/attempt55d-native-src
export_dir=${root}/export-attempt55d
raw=${root}/raw-attempt55d-native
cache=${root}/cache-attempt55d-native
inputs=${raw}/native-inputs
outputs=${raw}/native-outputs
air=${export_dir}/qwen_layer0_boundary_attempt55d.air
reference=${export_dir}/attempt55d-eager-reference.npz
baseline_reference=${root}/export-attempt55a/attempt55a-eager-reference.npz
baseline_native=${root}/raw-attempt55a-native/attempt55a-native-output.npz
...
g++ -D_GLIBCXX_USE_CXX11_ABI=0 -O2 -std=c++11 -ftrapv -fstack-protector-all \
  ...
  "${src}/native_layer0_attempt55d_host.cpp" -Wl,--whole-archive \
  ...
  -o "${raw}/native_layer0_attempt55d_host" >"${raw}/compile.log" 2>&1
s=$?; printf 'compile\t%s\n' "$s" >>"${status}"; [[ $s -eq 0 ]] || exit $s

timeout 1200 "${raw}/native_layer0_attempt55d_host" "${air}" "${inputs}" "${outputs}" \
...
python "${src}/compare_native.py" --native-dir "${outputs}" --reference "${reference}" \
  ...
  --output-npz "${raw}/attempt55d-native-output.npz" \
  --output "${raw}/attempt55d-result.json" >"${raw}/compare.stdout.log" 2>&1
compare_status=$?
printf 'compare\t%s\n' "${compare_status}" >>"${status}"

python "${src}/compare_delta.py" --baseline-native "${baseline_native}" \
  --baseline-reference "${baseline_reference}" \
  --candidate-native "${raw}/attempt55d-native-output.npz" \
  --candidate-reference "${reference}" \
  --output "${raw}/attempt55d-vs-55a-delta.json" >"${raw}/delta.stdout.log" 2>&1
delta_status=$?
printf 'materialize-hypothesis\t%s\n' "${delta_status}" >>"${status}"
...
if [[ ${materialize_status} -eq 0 && ${materialize_count} -eq 8 ]]; then
  materialize_gate=0
else
  materialize_gate=92
fi
...
sha256sum "${air}" "${raw}/attempt55d-native-output.npz" \
  "${raw}/attempt55d-result.json" "${raw}/attempt55d-vs-55a-delta.json" \
  >"${raw}/result-integrity.log" 2>/dev/null || true
cat "${raw}/compare.stdout.log"
cat "${raw}/delta.stdout.log"
...
if [[ ${delta_status} -ne 0 ]]; then exit ${delta_status}; fi
exit ${idle_status}

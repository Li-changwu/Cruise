...
src=${root}/attempt61a-native-src
export_dir=${root}/export-attempt61a
raw=${root}/raw-attempt61a-native
inputs=${raw}/native-inputs
outputs=${raw}/native-outputs
air=${export_dir}/qwen_full_decoder_step_attempt61a.air
...
g++ -D_GLIBCXX_USE_CXX11_ABI=0 -O2 -std=c++11 -ftrapv -fstack-protector-all \
  ...
  "${src}/native_full_decoder_attempt61a_host.cpp" -Wl,--whole-archive \
  ...
  -o "${raw}/native_full_decoder_attempt61a_host" >"${raw}/compile.log" 2>&1
s=$?
printf 'compile\t%s\n' "$s" >>"${status}"
[[ $s -eq 0 ]] || exit $s

timeout 3600 "${raw}/native_full_decoder_attempt61a_host" "${air}" "${inputs}" \
...
python "${src}/compare_native.py" --native-dir "${outputs}" --reference "${reference}" \
  --initial-input-dir "${inputs}" --output-npz "${raw}/attempt61a-native-output.npz" \
  --output "${raw}/attempt61a-result.json" >"${raw}/compare.stdout.log" 2>&1
compare_status=$?
...
sha256sum "${air}" "${raw}/attempt61a-native-output.npz" \
  "${raw}/attempt61a-result.json" "${raw}/linear-launches.json" \
  >"${raw}/result-integrity.log" 2>/dev/null || true
cat "${raw}/attempt61a-result.json" 2>/dev/null || true

...
int main(int argc, char **argv) {
  if (argc != 4) {
    std::cerr << "usage: native_full_decoder_attempt61a_host AIR INPUT_DIR OUTPUT_DIR\n";
    return 2;
  }

  ge::Graph graph("G4aAttempt61aCompleteDecoder");
  const auto load_status = graph.LoadFromFile(argv[1]);
  std::cout << "G4A_ATTEMPT61A_LOAD status=" << load_status
  ...
  for (int step = 1; step <= 4; ++step) {
    ...
    std::cout << "G4A_ATTEMPT61A_STEP step=" << step << " outputs=" << outputs.size()
              << std::endl;
  }

  ge::GEFinalize();
  std::cout << "G4A_ATTEMPT61A_NATIVE_COMPLETE" << std::endl;
  return 0;
}

...
run_id=${CRUISE_RUN_ID:-persistent-cancel-p2-$(date -u +%Y%m%dT%H%M%SZ)}
...
profile_enabled=${CRUISE_P2_PROFILE:-1}
run_mode=${CRUISE_P2_MODE:-full}
ascend_log_level=${CRUISE_ASCEND_LOG_LEVEL:-3}
placement_log_level=${CRUISE_P2_PLACEMENT_LOG_LEVEL:-0}
...
host_source=${script_dir}/persistent_cancel_p2_host.cpp
analyzer=${script_dir}/analyze_p2.py
placement_analyzer=${script_dir}/placement_evidence.py
config_writer=${script_dir}/prepare_p2_config.py
...
[[ "${run_mode}" == full || "${run_mode}" == placement-diagnostic ||
   "${run_mode}" == single-start-diagnostic ]]
...
for required in "${conda_sh}" "${cann_set_env}" "${guard}" "${template}" \
  ...
  "${resource_writer}" "${controller_source}/CMakeLists.txt" \
  "${controller_source}/persistent_cancel_p2.cpp"; do
  [[ -f "${required}" ]] || {
...
finalize() {
  ...
  trap - EXIT
  set +e
  if [[ ${command_status} -ne 0 && -n "${driver_logs:-}" && -d "${driver_logs}" ]]; then
    mkdir -p "${evidence}/failure-driver-logs"
    cp -a "${driver_logs}/." "${evidence}/failure-driver-logs/"
  fi
...
cp "${controller_source}/CMakeLists.txt" \
  "${controller_source}/persistent_cancel_p2.cpp" \
  "${controller_workspace}/"
...
[[ -f "${ASCEND_OPP_PATH}/built-in/op_proto/inc/array_ops.h" &&
   -f "${ASCEND_OPP_PATH}/built-in/op_proto/inc/elewise_calculation_ops.h" ]] || {
  printf 'missing GraphPp operator headers under ASCEND_OPP_PATH: %s\n' \
    "${ASCEND_OPP_PATH}" >&2
  exit 96
}
...
function_config=${config_dir}/persistent_cancel_p2_func.json
graph_config=${config_dir}/persistent_cancel_p2_graph.json
toolchain_config=${config_dir}/persistent_cancel_p2_toolchain.json
deploy_config=${config_dir}/persistent_cancel_p2_deploy.json
python "${config_writer}" --workspace "${controller_workspace}" \
  --ascend-toolchain "${ascend_toolchain}" \
  --function-output "${function_config}" --graph-output "${graph_config}" \
  --toolchain-output "${toolchain_config}" --deploy-output "${deploy_config}"

host_binary=${build}/persistent_cancel_p2_host
g++ -D_GLIBCXX_USE_CXX11_ABI=0 -O2 -std=c++11 -ftrapv \
  -fstack-protector-all -pthread \
  -I"${ASCEND_HOME_PATH}/include" \
  -I"${ASCEND_HOME_PATH}/include/external" \
  -I"${ASCEND_OPP_PATH}/built-in/op_proto/inc" \
  "${host_source}" \
  -Wl,--whole-archive \
...
sha256sum "${host_binary}" "${host_source}" \
  "${controller_workspace}/persistent_cancel_p2.cpp" \
  "${function_config}" "${graph_config}" "${toolchain_config}" \
  "${deploy_config}" "${RESOURCE_CONFIG_PATH}" \
...
storage_guard_run_log "${evidence}/placement-probe.log" \
  "${evidence}/placement-probe.meta.json" 600 -- \
  "${host_binary}" "${function_config}" "${graph_config}" "${deploy_config}" \
  9001 placement_probe
...
export ASCEND_GLOBAL_LOG_LEVEL=${ascend_log_level}
review_intermediates after-placement
if [[ ${placement_status} -ne 0 ]]; then
  printf 'P2 placement gate failed; see %s\n' "${placement_result}" >&2
  exit "${placement_status}"
fi
if [[ "${run_mode}" == placement-diagnostic ]]; then
  printf 'PERSISTENT_CANCEL_P2_PLACEMENT_COMPLETE evidence=%s\n' "${evidence}"
  exit 0
fi

start_count=3
if [[ "${run_mode}" == single-start-diagnostic ]]; then
  start_count=1
fi
for start in $(seq 1 "${start_count}"); do
  owner=$((1000 + start))
  storage_guard_run_log "${evidence}/start-${start}.log" \
    "${evidence}/start-${start}.meta.json" 1800 -- \
    "${host_binary}" "${function_config}" "${graph_config}" "${deploy_config}" \
    "${owner}"
  wait_for_npu_release "after-start-${start}"
  review_intermediates "after-start-${start}"
done
if [[ "${run_mode}" == single-start-diagnostic ]]; then
  printf 'PERSISTENT_CANCEL_P2_SINGLE_START_COMPLETE evidence=%s\n' \
    "${evidence}"
  exit 0
fi
...
if [[ "${profile_enabled}" == 1 ]]; then
  ...
  storage_guard_run_log "${evidence}/profile.log" \
    "${evidence}/profile.meta.json" 1800 -- \
  ...
  review_intermediates after-profile
fi
...
if [[ ${analysis_status} -eq 0 ]]; then
  storage_guard_assert_scratch_path "${driver_logs}"
  find "${driver_logs}" -type f -delete
  review_intermediates after-analysis
else
  exit "${analysis_status}"
fi
if [[ "${profile_enabled}" == 0 ]]; then
  printf 'PERSISTENT_CANCEL_P2_MECHANISM_COMPLETE profile=skipped evidence=%s\n' \
    "${evidence}"
else
  printf 'PERSISTENT_CANCEL_P2_COMPLETE evidence=%s\n' "${evidence}"
fi

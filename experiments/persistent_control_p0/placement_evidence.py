#!/usr/bin/env python3
"""Extract fail-closed P0 controller placement evidence from CANN logs."""
...
FORMAT = "cruise-persistent-control-p0-placement-v1"
...
_PLACEMENT_PROBE = re.compile(
    r"P0_PLACEMENT_PROBE .*feed_calls=2 commits=1 quiescent=1 .*"
    r"compile_status=0 fetch_status=0 remove_status=0 finalize_status=0"
)
_FORBIDDEN = {
...
def analyze_placement(log_roots: list[Path]) -> dict[str, Any]:
        ...
        with path.open(encoding="utf-8", errors="replace") as stream:
            for line_number, raw_line in enumerate(stream, 1):
                ...
                if runnable is not None:
                    ...
                    matched = True
                model_device_type = _MODEL_DEVICE_TYPE.search(line)
                if model_device_type is not None:
                    model_device_types.append(int(model_device_type.group(1)))
                    matched = True
    ...
    normalized_selected = [value.strip().lower() for value in selected_resources]
    errors: list[str] = []
    if not node_types:
        errors.append(f"no deployment record for {TARGET_PROCESS_POINT}")
    if "ascend" not in normalized_selected:
        errors.append(f"Ascend resource selection is not proven: {selected_resources}")
    if not model_device_types or any(value != 0 for value in model_device_types):
        errors.append(f"controller Device model type is not proven: {model_device_types}")
    if device_proxy_records == 0:
        errors.append("Device UDF proxy deployment is not proven")
    if placement_probe_records != 1:
        errors.append(
            f"successful one-quantum placement probes={placement_probe_records}, expected 1"
        )
    present_forbidden = {
        ...
        json.dumps(
            {
                ...
                "deployment_node_types": result["deployment_node_types"],
                "selected_resource_types": result["selected_resource_types"],
                "model_device_types": result["model_device_types"],
                "device_proxy_records": result["device_proxy_records"],
                "placement_probe_records": result["placement_probe_records"],
                "forbidden_evidence_counts": result["forbidden_evidence_counts"],

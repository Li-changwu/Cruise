...
run_id=${CRUISE_RUN_ID:-persistent-owner-v2-device-kv-update-$(date -u +%Y%m%dT%H%M%SZ)}
persistent_root=${CRUISE_PERSISTENT_ROOT:-/workspace/cruise-runs}
run_root=${persistent_root}/${run_id}
evidence=${run_root}/evidence
scratch=${CRUISE_V2_DEVICE_KV_SCRATCH:-/dev/shm/cruise-v2-device-kv-update-${physical_npu}-$$}
python_bin=${CRUISE_V2_DEVICE_KV_PYTHON:-$(command -v python3)}
...
host_source=${script_dir}/device_kv_update_probe_host.cpp
verifier=${script_dir}/verify_probe.py
controller_source=${script_dir}/controller
op_kernel=${script_dir}/op_kernel/device_kv_slot_update.cpp
op_def=${script_dir}/op_host/device_kv_slot_update_def.cpp
op_infershape=${script_dir}/op_host/device_kv_slot_update_infershape.cpp
op_tiling=${script_dir}/op_host/device_kv_slot_update_tiling.cpp
...
for required in "${python_bin}" "${cann_set_env}" "${guard}" \
  ...
  "${config_writer}" "${host_source}" "${verifier}" "${op_kernel}" \
  "${op_def}" "${op_infershape}" "${op_tiling}" \
  "${controller_source}/CMakeLists.txt" \
  "${controller_source}/device_kv_update_controller.cpp"; do
  [[ -f "${required}" ]] || {
    printf 'missing V2 Device KV update probe input: %s\n' "${required}" >&2
    exit 96
  }
done
[[ "${scratch}" == /dev/shm/cruise-v2-device-kv-update-* ]] || {
  printf 'V2 Device KV scratch must be PID-scoped under /dev/shm: %s\n' \
    "${scratch}" >&2
  exit 96
}
...
cp "${op_kernel}" "${project}/op_kernel/"
cp "${op_def}" "${op_infershape}" "${op_tiling}" "${project}/op_host/"
cp "${controller_source}/CMakeLists.txt" \
  "${controller_source}/device_kv_update_controller.cpp" \
...
cmake -S "${project}" -B "${project}/build_out" -G 'Unix Makefiles' \
  ...
  -Dvendor_name=cruise_device_kv_update_probe \
...
"${python_bin}" - "${export_dir}/export-result.json" \
  "${export_dir}/device_kv_update_probe.air" \
...
g++ -D_GLIBCXX_USE_CXX11_ABI=0 -O2 -std=c++11 -ftrapv \
  ...
  -Wl,--no-whole-archive -o "${build}/device_kv_update_probe_host" \
...
for mode in graph dataflow; do
  ...
  storage_guard_run_log "${evidence}/${mode}.log" \
    ...
    "${build}/device_kv_update_probe_host" "${mode}" \
    "${export_dir}/device_kv_update_probe.air" "${config_dir}/graph.json" \
  ...
  find "${mode_driver_logs}" -type f -print0 | sort -z | \
    xargs -0 -r rg -i \
      'LaunchKernel: kernel info.*kernel_name=te_devicekvslotupdate_' \
...
sha256sum "${package}" "${export_dir}/device_kv_update_probe.air" \
...
sha256sum "${exporter}" "${inspector}" "${config_writer}" "${host_source}" \
  ...
  "${controller_source}"/* "${script_dir}/op_host"/* \
  "${script_dir}/op_kernel"/* >"${evidence}/source-identity.sha256"
...
verifier_status=$?
set -e
storage_guard_snapshot device-kv-update-complete \
  "${evidence}/storage-device-kv-update-complete.tsv"

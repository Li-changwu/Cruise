#!/usr/bin/env python3
"""Render P1 FunctionPp, GraphPp, toolchain, and deployment configs."""
...
def make_function_config(workspace: Path, toolchain_config: Path) -> dict:
    ...
    if not (workspace / "persistent_recurrence_p1.cpp").is_file():
        raise ValueError("controller workspace has no P1 source")
    return {
        "func_list": [
            {
        ...
        "input_num": 1,
        "output_num": 1,
        "target_bin": "libpersistent_recurrence_p1.so",
...
def make_graph_config() -> dict:
    return {
        "build_options": {
            "ge.modelFileNamePrefix": "persistent_recurrence_p1_add"
        },
        "inputs_tensor_desc": [

#!/usr/bin/env python3
"""Extract fail-closed P1 controller placement evidence from CANN logs."""
...
FORMAT = "cruise-persistent-recurrence-p1-placement-v1"
...
_PLACEMENT_PROBE = re.compile(
    r"P1_PLACEMENT_PROBE .*feed_calls=2 commits=256 aicore_calls=256 "
    r"quiescent=1 .*compile_status=0 fetch_status=0 remove_status=0 "
    r"finalize_status=0"
)
_FORBIDDEN = {
...
def analyze_placement(log_roots: list[Path]) -> dict[str, Any]:
    ...
    if placement_probe_records != 1:
        errors.append(
            f"successful P1 placement probes={placement_probe_records}, expected 1"
        )
    present_forbidden = {

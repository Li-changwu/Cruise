...
run_id=${CRUISE_RUN_ID:-persistent-recurrence-p1-$(date -u +%Y%m%dT%H%M%SZ)}
...
profile_enabled=${CRUISE_P1_PROFILE:-1}
run_mode=${CRUISE_P1_MODE:-full}
ascend_log_level=${CRUISE_ASCEND_LOG_LEVEL:-3}
placement_log_level=${CRUISE_P1_PLACEMENT_LOG_LEVEL:-0}
...
host_source=${script_dir}/persistent_recurrence_p1_host.cpp
analyzer=${script_dir}/analyze_p1.py
placement_analyzer=${script_dir}/placement_evidence.py
config_writer=${script_dir}/prepare_p1_config.py
...
for required in "${conda_sh}" "${cann_set_env}" "${guard}" "${template}" \
  ...
  "${controller_source}/persistent_recurrence_p1.cpp"; do
  [[ -f "${required}" ]] || {
...
cp "${controller_source}/CMakeLists.txt" \
  "${controller_source}/persistent_recurrence_p1.cpp" \
  "${controller_workspace}/"
...
function_config=${config_dir}/persistent_recurrence_p1_func.json
graph_config=${config_dir}/persistent_recurrence_p1_graph.json
toolchain_config=${config_dir}/persistent_recurrence_p1_toolchain.json
deploy_config=${config_dir}/persistent_recurrence_p1_deploy.json
...
host_binary=${build}/persistent_recurrence_p1_host
...
sha256sum "${host_binary}" "${host_source}" \
  "${controller_workspace}/persistent_recurrence_p1.cpp" \
...
if [[ ${placement_status} -ne 0 ]]; then
  printf 'P1 placement gate failed; see %s\n' "${placement_result}" >&2
  exit "${placement_status}"
fi
if [[ "${run_mode}" == placement-diagnostic ]]; then
  printf 'PERSISTENT_RECURRENCE_P1_PLACEMENT_COMPLETE evidence=%s\n' "${evidence}"
  exit 0
fi
...
for start in 1 2 3; do
  ...
  storage_guard_run_log "${evidence}/start-${start}.log" \
    "${evidence}/start-${start}.meta.json" 1200 -- \
    "${host_binary}" "${function_config}" "${graph_config}" "${deploy_config}" \
    "${owner}"
...
if [[ "${profile_enabled}" == 0 ]]; then
  printf 'PERSISTENT_RECURRENCE_P1_MECHANISM_COMPLETE profile=skipped evidence=%s\n' \
    "${evidence}"
else
  printf 'PERSISTENT_RECURRENCE_P1_COMPLETE evidence=%s\n' "${evidence}"
fi

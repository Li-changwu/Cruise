...
class SidecarDataFlowEngine:
    def __init__(self) -> None:
        ...
        trace_path_raw = os.getenv("VLLM_ASCEND_RESIDENT_EPOCH_MEMCPY_TRACE_PATH")
        if bool(trace_library_raw) != bool(trace_path_raw):
            raise RuntimeError("memcpy trace library and output path must be set together")
        if trace_library_raw and trace_path_raw:
            trace_library = Path(trace_library_raw).resolve()
            trace_path = Path(trace_path_raw).resolve()
            if not trace_library.is_file():
                raise RuntimeError(f"memcpy trace library is missing: {trace_library}")
            if not str(trace_library).startswith("/dev/shm/"):
                raise RuntimeError("memcpy trace library must be in /dev/shm")
            if not str(trace_path).startswith("/dev/shm/"):
                raise RuntimeError("memcpy trace output must be in /dev/shm")
            if trace_path.exists():
                raise RuntimeError(f"memcpy trace output already exists: {trace_path}")
            trace_path.parent.mkdir(parents=True, exist_ok=True)
            existing_preload = child_env.get("LD_PRELOAD")
            child_env["LD_PRELOAD"] = (
                f"{trace_library}:{existing_preload}"
                if existing_preload
                else str(trace_library)
            )
            child_env["ASCEND_RT_MEMCPY_TRACE_PATH"] = str(trace_path)

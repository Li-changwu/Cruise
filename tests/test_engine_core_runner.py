...
from run_multi_epoch_cohort import run_dynamic_trace
...
def test_engine_core_resolves_resident_scheduler_worker_and_epoch(monkeypatch):
    ...
    monkeypatch.setenv("VLLM_ASCEND_RESIDENT_EPOCH_STEPS", "4")
    monkeypatch.setenv("VLLM_ASCEND_RESIDENT_EPOCH_CAPACITY", "8")
...
def test_engine_core_multi_epoch_cohort_and_warmup(monkeypatch):
    monkeypatch.setenv(
        "VLLM_ASCEND_RESIDENT_EPOCH_BACKEND_FACTORY",
        "vllm_ascend_resident_epoch.testing:create_test_engine",
    )
    monkeypatch.setenv("VLLM_ASCEND_RESIDENT_EPOCH_STEPS", "2")
    monkeypatch.setenv("VLLM_ASCEND_RESIDENT_EPOCH_CAPACITY", "8")
    config = make_engine_vllm_config(MODEL_CONFIG)
    engine_core = EngineCore(
        vllm_config=config,
    ...
    try:
        ...
        assert worker.backend.engine.warmup_calls == 1

        trace = run_dynamic_trace(
            engine_core=engine_core,
            expected_tokens=[11691, 11692, 11693, 11694, 11695, 11696],
            input_token_id=11690,
        )
        assert trace["pass"], trace
        assert worker.backend.engine.execute_calls == 3
    finally:
        engine_core.shutdown()
